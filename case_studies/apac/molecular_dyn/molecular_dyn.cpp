...
Cell cell_create(int capacity) {
  ...
  new_cell.particles_symb =
      (Particle_symb*)malloc(capacity * sizeof(Particle_symb));
  if (!new_cell.particles_symb) {
    return new_cell;
  }
  new_cell.particles_forces =
      (Particle_forces*)malloc(capacity * sizeof(Particle_forces));
...
void cell_add_particle(Cell* cell, Particle_symb particle_symb,
  ...
  if (cell->size == cell->capacity) {
    ...
    cell->particles_symb = (Particle_symb*)realloc(
        cell->particles_symb, new_capacity * sizeof(Particle_symb));
    if (!cell->particles_symb) {
      return;
    }
    cell->particles_forces = (Particle_forces*)realloc(
        cell->particles_forces, new_capacity * sizeof(Particle_forces));
    if (!cell->particles_forces) {
      return;
    }
...
void cell_self_compute(const Particle_symb* particles_symb,
                       Particle_forces* particles_forces, const int size) {
  for (int idxTgt = 0; idxTgt < size; idxTgt++) {
    for (int idxSrc = 0; idxSrc < idxTgt + 1; idxSrc++) {
      ...
      const double coef = particles_symb[idxTgt].weight *
                          particles_symb[idxSrc].weight / cube_distance;
...
void cell_neighbor_compute(const Particle_symb* particles_symb,
                           Particle_forces* particles_forces, const int size,
                           const Particle_symb* particles_symbNeigh,
                           const int sizeNeighbor) {
  for (int idxTgt = 0; idxTgt < size; idxTgt++) {
    for (int idxSrc = 0; idxSrc < sizeNeighbor; idxSrc++) {
      const double dx =
          particles_symbNeigh[idxSrc].x - particles_symb[idxTgt].x;
      const double dy =
          particles_symbNeigh[idxSrc].y - particles_symb[idxTgt].y;
      const double dz =
          particles_symbNeigh[idxSrc].z - particles_symb[idxTgt].z;
      const double square_distance = dx * dx + dy * dy + dz * dz + 1e-05;
      const double distance = sqrt(square_distance);
      const double cube_distance = square_distance * distance;
      const double coef = particles_symb[idxTgt].weight *
                          particles_symbNeigh[idxSrc].weight / cube_distance;
      const double fx = dx * coef;
      const double fy = dy * coef;
      const double fz = dz * coef;
      particles_forces[idxTgt].fx += fx;
...
void cell_update(Particle_symb* particles_symb,
                 const Particle_forces* particles_forces, const int size,
                 double time_step) {
  for (int idxPart = 0; idxPart < size; idxPart++) {
    particles_symb[idxPart].vx += particles_forces[idxPart].fx /
                                  particles_symb[idxPart].weight * time_step;
    particles_symb[idxPart].vy += particles_forces[idxPart].fy /
                                  particles_symb[idxPart].weight * time_step;
    particles_symb[idxPart].vz += particles_forces[idxPart].fz /
                                  particles_symb[idxPart].weight * time_step;
...
int grid_cell_idx_from_position(const double cell_width,
                                const int nb_cells_per_dim,
                                Particle_symb particle) {
  int x = (int)(particle.x / cell_width), y = (int)(particle.y / cell_width),
      z = (int)(particle.z / cell_width);
  return (x * nb_cells_per_dim + y) * nb_cells_per_dim + z;
}

int grid_create(double box_width, double cell_width,
                const Particle_symb* src_particles_symb,
                const Particle_forces* src_particles_forces, const int src_size,
...
#pragma omp taskgroup
  {
    ...
    *particles_forces =
        (Particle_forces**)calloc(capacity, sizeof(Particle_forces*));
    for (int idxPart = 0; idxPart < src_size; idxPart++) {
#pragma omp taskwait depend(inout : cell_idx)
      int* cell_idx = new int();
#pragma omp task default(shared)                               \
    depend(in                                                  \
           : cell_width, nb_cells_per_dim, src_particles_symb, \
             src_particles_symb[idxPart], cell_idx)            \
        depend(inout                                           \
               : cell_idx[0], sizes) firstprivate(idxPart)
      {
        *cell_idx = grid_cell_idx_from_position(cell_width, nb_cells_per_dim,
                                                src_particles_symb[idxPart]);
        (*sizes)[*cell_idx]++;
      }
...
#pragma omp taskwait depend(in                                       \
                            : idx_x, idx_y, idx_z, nb_cells_per_dim) \
    depend(inout                                                     \
           : cell_idx)
          const int cell_idx =
              (idx_x * nb_cells_per_dim + idx_y) * nb_cells_per_dim + idx_z;
          (*particles_symb)[cell_idx] =
              (Particle_symb*)calloc((*sizes)[cell_idx], sizeof(Particle_symb));
          (*particles_forces)[cell_idx] = (Particle_forces*)calloc(
              (*sizes)[cell_idx], sizeof(Particle_forces));
        }
      }
    }
    int* cpt;
...
#pragma omp task default(shared)                                          \
    depend(in                                                             \
           : cell_width, nb_cells_per_dim, src_particles_symb,            \
             src_particles_symb[idxPart], cell_idx) depend(inout          \
                                                           : cell_idx[0]) \
        firstprivate(idxPart)
      *cell_idx = grid_cell_idx_from_position(cell_width, nb_cells_per_dim,
                                              src_particles_symb[idxPart]);
#pragma omp taskwait depend(in : cell_idx[0], cell_idx)
#pragma omp taskwait depend(in                                               \
                            : cell_idx[0], cpt, idxPart, src_particles_symb, \
                              src_particles_symb[idxPart], cell_idx)         \
    depend(inout                                                             \
           : particles_symb)
      (*particles_symb)[*cell_idx][cpt[*cell_idx]] =
          src_particles_symb[idxPart];
#pragma omp taskwait depend(in                                                 \
                            : cell_idx[0], cpt, idxPart, src_particles_forces, \
                              src_particles_forces[idxPart], cell_idx)         \
    depend(inout                                                               \
           : particles_forces)
      (*particles_forces)[*cell_idx][cpt[*cell_idx]] =
          src_particles_forces[idxPart];
...
void grid_destroy(const int nb_cells_per_dim, int** sizes,
                  Particle_symb*** particles_symb,
                  Particle_forces*** particles_forces) {
  for (int idx_x = 0; idx_x < nb_cells_per_dim; idx_x++) {
    for (int idx_y = 0; idx_y < nb_cells_per_dim; idx_y++) {
...
void grid_compute(const int nb_cells_per_dim, const int* sizes,
                  const Particle_symb* const* particles_symb,
                  Particle_forces** particles_forces) {
#pragma omp taskgroup
  {
...
#pragma omp taskwait depend(in                                            \
                            : me, particles_forces, particles_forces[me]) \
    depend(inout                                                          \
           : me_particles_forces)
          Particle_forces* me_particles_forces = particles_forces[me];
#pragma omp task default(shared)                                \
    depend(in                                                   \
           : me, particles_symb, sizes, sizes[me]) depend(inout \
                                                          : particles_forces)
...
#pragma omp taskwait depend(in                                               \
                            : idx_x, idx_x_neigh, idx_y, idx_y_neigh, idx_z, \
                              idx_z_neigh, nb_cells_per_dim)                 \
    depend(inout                                                             \
           : neighbor)
                neighbor =
                    ((idx_x + idx_x_neigh + nb_cells_per_dim) %
                     ...
                     (idx_y + idx_y_neigh + nb_cells_per_dim) %
                         nb_cells_per_dim) *
                        nb_cells_per_dim +
                    (idx_z + idx_z_neigh + nb_cells_per_dim) % nb_cells_per_dim;
                const Particle_symb* neighbor_particles_symb =
                    particles_symb[neighbor];
#pragma omp task default(shared)                                              \
    depend(in                                                                 \
           : me, neighbor, particles_symb, sizes, sizes[me], sizes[neighbor]) \
        depend(inout                                                          \
               : particles_forces)
                cell_neighbor_compute(me_particles_symb, me_particles_forces,
                                      sizes[me], neighbor_particles_symb,
                                      sizes[neighbor]);
              }
            }
          }
...
void grid_update(const int nb_particles, const int nb_cells_per_dim,
                 const double box_width, const double cell_width,
                 double time_step, int** sizes, Particle_symb*** particles_symb,
                 Particle_forces*** particles_forces) {
#pragma omp taskgroup
  {
    int* src_sizes = *sizes;
...
#pragma omp taskwait depend(in                                       \
    ...
    depend(inout                                                     \
           : cell_idx)
          const int* const cell_idx = new const int(
              (idx_x * nb_cells_per_dim + idx_y) * nb_cells_per_dim + idx_z);
#pragma omp taskwait depend(inout : sizes)
          for (int idxPart = 0; idxPart < src_sizes[*cell_idx]; idxPart++) {
            src_particles_symb[*cell_idx][idxPart].vx +=
                src_particles_forces[*cell_idx][idxPart].fx /
                src_particles_symb[*cell_idx][idxPart].weight * time_step;
            src_particles_symb[*cell_idx][idxPart].vy +=
                src_particles_forces[*cell_idx][idxPart].fy /
                src_particles_symb[*cell_idx][idxPart].weight * time_step;
            src_particles_symb[*cell_idx][idxPart].vz +=
                src_particles_forces[*cell_idx][idxPart].fz /
                src_particles_symb[*cell_idx][idxPart].weight * time_step;
            src_particles_symb[*cell_idx][idxPart].x +=
                src_particles_symb[*cell_idx][idxPart].vx * time_step;
            src_particles_symb[*cell_idx][idxPart].y +=
                src_particles_symb[*cell_idx][idxPart].vy * time_step;
            src_particles_symb[*cell_idx][idxPart].z +=
                src_particles_symb[*cell_idx][idxPart].vz * time_step;
#pragma omp taskwait depend(in : cell_idx) depend(inout : particles_symb)
            while (src_particles_symb[*cell_idx][idxPart].x < 0) {
#pragma omp taskwait depend(in                              \
                            : box_width, cell_idx, idxPart) \
    depend(inout                                            \
           : particles_symb)
              src_particles_symb[*cell_idx][idxPart].x += box_width;
            }
#pragma omp taskwait depend(in                                  \
                            : box_width, cell_idx) depend(inout \
                                                          : particles_symb)
...
#pragma omp task default(shared) depend(                                   \
    in                                                                     \
    : cell_idx, cell_width, nb_cells_per_dim, particles_symb, up_cell_idx) \
    depend(inout                                                           \
           : sizes, up_cell_idx[0]) firstprivate(idxPart)
            {
              *up_cell_idx = grid_cell_idx_from_position(
                  cell_width, nb_cells_per_dim,
                  src_particles_symb[*cell_idx][idxPart]);
              (*sizes)[*up_cell_idx]++;
            }
...
#pragma omp task default(shared) depend(                                   \
    ...
    : cell_idx, cell_width, nb_cells_per_dim, particles_symb, up_cell_idx) \
    depend(inout                                                           \
           : up_cell_idx[0]) firstprivate(idxPart)
            *up_cell_idx = grid_cell_idx_from_position(
                cell_width, nb_cells_per_dim,
                src_particles_symb[*cell_idx][idxPart]);
#pragma omp taskwait depend(in : up_cell_idx[0], up_cell_idx)
#pragma omp taskwait depend(in                                        \
                            : cell_idx, cpt, idxPart, up_cell_idx[0], \
                              up_cell_idx) depend(inout               \
                                                  : particles_symb)
            (*particles_symb)[*up_cell_idx][cpt[*up_cell_idx]] =
                src_particles_symb[*cell_idx][idxPart];
#pragma omp taskwait depend(in                                        \
                            : cell_idx, cpt, idxPart, up_cell_idx[0], \
                              up_cell_idx) depend(inout               \
                                                  : particles_forces)
            (*particles_forces)[*up_cell_idx][cpt[*up_cell_idx]] =
                src_particles_forces[*cell_idx][idxPart];
#pragma omp taskwait depend(in                                          \
                            : up_cell_idx[0], up_cell_idx) depend(inout \
                                                                  : cpt)
            cpt[*up_cell_idx]++;
#pragma omp task default(shared) depend(inout : up_cell_idx)
            delete up_cell_idx;
...
#pragma omp task default(shared) depend(in                  \
                                        : nb_cells_per_dim) \
    depend(inout                                            \
           : particles_forces, particles_symb, sizes)
    grid_destroy(nb_cells_per_dim, &src_sizes, &src_particles_symb,
                 &src_particles_forces);
  __apac_exit:;
  }
}
...
#pragma omp task default(shared) depend(                                  \
    in                                                                    \
    : inCell, particle[0], particle_forces[0], particle, particle_forces) \
    depend(inout                                                          \
           : inCell[0])
...
#pragma omp task default(shared) depend(                                    \
    in                                                                      \
    : box_width, cell, cell_width, particles_forces, particles_symb, sizes) \
    depend(inout                                                            \
           : nb_cells_per_dim, particles_forces[0], particles_forces[0][0], \
             particles_symb[0], particles_symb[0][0], sizes[0])
    nb_cells_per_dim = grid_create(box_width, cell_width, cell.particles_symb,
                                   cell.particles_forces, cell.size, &sizes,
...
#pragma omp task default(shared)                                        \
    depend(in                                                           \
           : box_width, cell_width, nb_cells_per_dim, particles_forces, \
             particles_symb, size, sizes, time_step)                    \
        depend(inout                                                    \
               : particles_forces[0], particles_forces[0][0],           \
                 particles_symb[0], particles_symb[0][0], sizes[0])
      {
        grid_compute(nb_cells_per_dim, sizes, particles_symb, particles_forces);
        grid_update(size, nb_cells_per_dim, box_width, cell_width, time_step,
                    &sizes, &particles_symb, &particles_forces);
      }
    }
#pragma omp task default(shared)                                        \
    depend(in                                                           \
           : nb_cells_per_dim, particles_forces, particles_symb, sizes) \
        depend(inout                                                    \
               : particles_forces[0], particles_forces[0][0],           \
                 particles_symb[0], particles_symb[0][0], sizes[0])
