...
#pragma omp task default(shared) depend(in                          \
    ...
    depend(inout                                                    \
      ...
      int* pivot = new int();
#pragma omp task default(shared) depend(in                                 \
                                        : in_out_data, right_limit, pivot) \
    depend(inout                                                           \
           : in_out_data[0], pivot[0])
      partition(pivot, in_out_data, right_limit);
#pragma omp task default(shared) depend(in                              \
                                        : in_out_data, pivot[0], pivot) \
    depend(inout                                                        \
           : in_out_data[0])
      sort_core(&in_out_data[0], *pivot);
#pragma omp task default(shared)                        \
    depend(in                                           \
           : in_out_data, pivot[0], right_limit, pivot) \
        depend(inout                                    \
               : in_out_data[*pivot + 1])
...
#pragma omp taskgroup
  {
    ...
    unsigned int __apac_var1;
    __apac_var1 = time(NULL);
    srand(__apac_var1);
