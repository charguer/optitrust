...
let _  = Run.script_cpp ( fun _ ->
  ...
  let f = f in
  !! insert_max_tab ~tab:typed_tab ~size:(size)   [f; cVarDef "a"];
  !! Matrix_basic.local_name tab ~into:"w" ~type_and_dims:(typ_f32, [size]) [cFunBody "accesses"; cFor "i"];
  let w = trm_find_var "w" [f] in
  let address_pattern = trm_array_access w (trm_find_var "i" [f]) in

  !! Accesses.scale ~factor:(trm_find_var "sum" [f] ) ~address_pattern [tSpanSeq [cFunBody "accesses"] ];
