...
void matmul(int row_count, int col_count, int red_count, float *const x,
  ...
  for (int i = 0; i < row_count; i++) {
    __xwrites("for j in 0..col_count -> &x[MINDEX2(row_count, col_count, i, j)] ~> Cell");
...
void generate_prompt_proc(int vocabulary_len, int context_len, int layer_count,
  ...
  for (int l = 0; l < layer_count; l++) {
    ...
    __xreads("for i1 in 0..q_head_count -> for i2 in 0.. head_dim -> for i3 in "
             "0..embedding_dim -> &mha_q_weight[MINDEX4(layer_count, "
             "q_head_count, head_dim,embedding_dim,l,i1,i2,i3)] ~> Cell ");
    for (int i = 0; i < sequence_len; i++) {
      __strict();
      __sreads("mha_norm_weight ~> Matrix2(layer_count, embedding_dim)");
      __xmodifies(
      ...
      __sreads("for i1 in 0..q_head_count -> for i2 in 0.. head_dim -> for i3 "
               "in 0..embedding_dim -> &mha_q_weight[MINDEX4(layer_count, "
               "q_head_count, head_dim,embedding_dim,l,i1,i2,i3)] ~> Cell ");
...
//     float fci = _sinf(val);
//     __ghost([&]() {
//       __consumes("for i1 in j..(j + 1) -> &x[MINDEX1(col_count, i1)] ~>
//       Cell");
//       __produces("&x[MINDEX1(col_count, j)] ~> Cell");
//       __produces("&x[MINDEX1(col_count, j + 1)] ~> Cell");
//       __admitted();
...
//       __consumes("&x[MINDEX1(col_count, j)] ~> Cell");
//       __consumes("&x[MINDEX1(col_count, j + 1)] ~> Cell");
//       __produces("for i1 in j..(j + 1) -> &x[MINDEX1(col_count, i1)] ~>
//       Cell");
//       __admitted();
//     });
//     __ghost_end(focus_subrange);
...
//   __modifies("x ~> Matrix1(col_count)");
//   __ghost(assume, "P := in_range(0, 0..col_count)");
//   const __ghost_fn max = __ghost_begin(ro_matrix1_focus, "matrix := x, i :=
//   0"); float max_val = x[MINDEX1(col_count, 0)];
//   __ghost_end(max);
//   const __ghost_fn focus_subrange =
//       __ghost_begin(group_focus_subrange, "sub_range := 1..col_count");
//   for (int j = 1; j < col_count; j++) {
...
// void forward(int token, int vocabulary_len, int context_len, int layer_count,
//              int q_head_count, int kv_head_count, int
//              q_head_per_kv_head_count, int embedding_dim, int head_dim, int
//              q_dim, int kv_dim, int hidden_dim, float epsilon, float*
//              embedding_weight, float* mha_norm_weight, float* mha_q_weight,
//              float* mha_k_weight, float* mha_v_weight, float* mha_out_weight,
//              float* ffn_norm_weight, float* ffn_fc_weight, float*
//              ffn_up_weight, float* ffn_out_weight, float* out_norm_weight,
//              float* out_weight, float* k_cache, float* v_cache, float*
//              logits, int pos, int logits_count) {
//   __reads("embedding_weight ~> Matrix2(vocabulary_len, embedding_dim)");
//   __reads("mha_norm_weight ~> Matrix2(layer_count, embedding_dim)");
//   __reads(
...
//   float* const embedding =
//       (float*)malloc(MSIZE1(embedding_dim) * sizeof(float));
//   float* const mha_norm = (float*)malloc(MSIZE1(embedding_dim) *
//   sizeof(float)); float* const mha_q =
//       (float*)malloc(MSIZE2(q_head_count, head_dim) * sizeof(float));
//   __ghost(assume, "P := in_range(token, 0..vocabulary_len)");
//   for (int e = 0; e < embedding_dim; e++) {
//     __strict();
...
// void generate_prompt_proc(int vocabulary_len, int context_len, int
// layer_count,
//                           int q_head_count, int kv_head_count,
//                           int q_head_per_kv_head_count, int embedding_dim,
//                           int head_dim, int q_dim, int kv_dim, int
//                           hidden_dim, float epsilon, float* embedding_weight,
//                           float* mha_norm_weight, float* mha_q_weight,
//                           float* mha_k_weight, float* mha_v_weight,
//                           float* mha_out_weight, float* ffn_norm_weight,
//                           float* ffn_fc_weight, float* ffn_up_weight,
...
//           "&embedding[MINDEX2(sequence_len, embedding_dim, i, e)] ~> Cell");
//       const __ghost_fn focus_embedding_weight = __ghost_begin(
//           ro_matrix2_focus, "matrix := embedding_weight, i := token, j :=
//           e");
//       embedding[MINDEX2(sequence_len, embedding_dim, i, e)] =
//           embedding_weight[MINDEX2(vocabulary_len, embedding_dim, token, e)];
//       __ghost_end(focus_embedding_weight);
...
//   for (int l = 0; l < layer_count; l++) {
//     __strict();
//     __smodifies("mha_q ~> UninitMatrix3(sequence_len, q_head_count,
//     head_dim)");
//     __smodifies("mha_norm ~> UninitMatrix2(sequence_len, embedding_dim)");
//     __sreads("embedding ~> Matrix2(sequence_len, embedding_dim)");
//     __sreads("mha_norm_weight ~> Matrix2(layer_count, embedding_dim)");
...
//           "embedding_dim, l, q, h, e)] ~> Cell");
//       rmsnorm(
//           embedding_dim, &mha_norm[MINDEX2(sequence_len, embedding_dim, i,
//           0)], &embedding[MINDEX2(sequence_len, embedding_dim, i, 0)],
//           &mha_norm_weight[MINDEX2(layer_count, embedding_dim, l, 0)],
//           epsilon);
//       for (int q = 0; q < q_head_count; q++) {
//         __strict();
...
//             "embedding_dim, l, q, h, e)] ~> Cell");
//         matvec(head_dim, embedding_dim,
//                &mha_q[MINDEX3(sequence_len, q_head_count, head_dim, i, q,
//                0)], &mha_norm[MINDEX2(sequence_len, embedding_dim, i, 0)],
//                &mha_q_weight[MINDEX4(layer_count, q_head_count, head_dim,
//                                      embedding_dim, l, q, 0, 0)]);
//       }
