...
void matmul(int row_count, int col_count, int red_count, float* x, float* y,
            float* w) {
  ...
  for (int i = 0; i < row_count; i++) {
    __strict();
    __sreads("y ~> Matrix2(row_count, red_count)");
    __sreads("w ~> Matrix2(col_count, red_count)");
    __xwrites(
        "for j in 0..col_count -> &x[MINDEX2(row_count, col_count, i, j)] ~> "
        "Cell");
    for (int j = 0; j < col_count; j++) {
      __strict();
      __sreads("y ~> Matrix2(row_count, red_count)");
      ...
      for (int k = 0; k < red_count; k++) {
        __strict();
        __smodifies("&x[MINDEX2(row_count, col_count, i, j)] ~> Cell");
        __sreads("y ~> Matrix2(row_count, red_count)");
        __sreads("w ~> Matrix2(col_count, red_count)");
        const __ghost_fn focusy =
            __ghost_begin(ro_matrix2_focus, "matrix := y, i := i, j := k");
        const __ghost_fn focusw =
            __ghost_begin(ro_matrix2_focus, "matrix := w, i := j, j := k");
...
void generate_prompt_proc(int vocabulary_len, int context_len, int layer_count,
                          ...
                          const int q_head_per_kv_head_count, int embedding_dim,
  ...
  __reads(
      "mha_q_weight ~> Matrix4(layer_count, q_head_count, head_dim, "
      "embedding_dim)");
  const int kv_head_count = kv_head_count;
  const int q_head_per_kv_head_count = q_head_per_kv_head_count;
  float* const embedding =
  ...
  __ghost([&]() {
    __consumes("mha_q ~> UninitMatrix3(q_head_count, sequence_len, head_dim)");
    __produces(
        "for i1 in 0..sequence_len -> for i2 in 0..q_head_count -> for i3 in "
        "0..head_dim -> &mha_q[MINDEX3(q_head_count, sequence_len, head_dim, "
        "i2, i1, i3)] ~> UninitCell");
    __admitted();
  ...
  for (int l = 0; l < layer_count; l++) {
    __strict();
    __smodifies(
        "for i in 0..sequence_len -> for i1 in 0..q_head_count -> for i2 in "
        "0..head_dim -> &mha_q[MINDEX3(q_head_count, sequence_len, head_dim, "
        "i1, i, i2)] ~> UninitCell");
    ...
    __ghost(swap_groups,
            "outer_range := 0..sequence_len, inner_range := 0..q_head_count, "
            "items := fun (i: int) (q: int) -> for i1 in 0..head_dim -> "
            "&mha_q[MINDEX3(q_head_count, sequence_len, head_dim, q, i, i1)] "
            "~> UninitCell");
    for (int q = 0; q < q_head_count; q++) {
      __strict();
      ...
      __sreads(
          ...
          "embedding_dim)");
      __xmodifies(
          "for i in 0..sequence_len -> for i1 in 0..head_dim -> "
          "&mha_q[MINDEX3(q_head_count, sequence_len, head_dim, q, i, i1)] ~> "
          "UninitCell");
      for (int i = 0; i < sequence_len; i++) {
        __strict();
        ...
        __sreads(
            ...
            "embedding_dim)");
        __xmodifies(
            "for i1 in 0..head_dim -> &mha_q[MINDEX3(q_head_count, "
            "sequence_len, head_dim, q, i, i1)] ~> UninitCell");
        __xreads(
            "&mha_norm[MINDEX2(sequence_len, embedding_dim, i, 0)] ~> "
            "Matrix1(embedding_dim)");
        const __ghost_fn __ghost_pair_3 = __ghost_begin(
            ...
            "h in 0..head_dim -> for e in 0..embedding_dim -> "
            "&mha_q_weight[MINDEX4(layer_count, q_head_count, head_dim, "
            "embedding_dim, l, q, h, e)] ~> Cell");
        const __ghost_fn __ghost_pair_5 = __ghost_begin(
            ro_group_focus,
            "i := q, items := fun (q: int) -> for h in 0..head_dim -> for e in "
    ...
    __ghost(swap_groups,
            "outer_range := 0..q_head_count, inner_range := 0..sequence_len, "
            "items := fun (q: int) (i: int) -> for i1 in 0..head_dim -> "
            "&mha_q[MINDEX3(q_head_count, sequence_len, head_dim, q, i, i1)] "
            "~> UninitCell");
  }
  __ghost([&]() {
    __consumes(
        ...
        "0..head_dim -> &mha_q[MINDEX3(q_head_count, sequence_len, head_dim, "
        "i2, i1, i3)] ~> UninitCell");
    __produces("mha_q ~> UninitMatrix3(q_head_count, sequence_len, head_dim)");
    __admitted();
    __with("justif := reorder_groups");
  });
  free(mha_q);
