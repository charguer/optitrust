...
let quantized_var var = new_var ("q_" ^var.name)
(* There are three things to change when quantizing :
- the var def : should be casted to a new var with int* type
- the accumulation : should be transformed with quantized accumulation
- the de quantiation : should happens on a set operation; we need to add multiplication *)
