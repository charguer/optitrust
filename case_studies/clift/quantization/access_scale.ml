...
let insert_max_tab_on (tab : typed_var) (size : trm) (i : int) (t : trm) : trm =
  ...
  let div = trm_exact_div ~typ (trm_float ~typ:typ_f32 1.0) call_max in
...
let insert_max_tab ~(tab : typed_var) ~(size : trm) (tg : target) : unit = apply_at_target_paths_in_seq (insert_max_tab_on tab size) tg
...
let _ =
  Run.script_cpp (fun _ ->
      ...
      let f = f in
      !!insert_max_tab ~tab:typed_tab ~size [ f; cVarDef "b" ];
