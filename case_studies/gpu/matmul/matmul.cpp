...
__ghost(to_prove, "MSIZE2(8, 4) = MSIZE2(exact_div(32, 8), exact_div(32, 4))");
__ghost(to_prove, "MSIZE2(exact_div(32, 8), exact_div(32, 4)) = MSIZE2(8, 4)");
// ----
...
void mm(float* c, float* a, float* b, int m, int n, int p) {
  ...
  // FIXME:
  __ghost(to_prove, "exact_div(m, 32) >= 0");
  __ghost(to_prove, "exact_div(n, 32) >= 0");
  // ----
  ...
  for (int i = 0; i < m; i++) {
    ...
    for (int j = 0; j < n; j++) {
      ...
      for (int k = 0; k < p; k++) {
        ...
        __ghost(rewrite_float_linear, "inside := fun v -> &sum ~~> v, by := reduce_sum_add_right(k, fun k -> A(i, k) *. B(k, j), k_ge_0)");
      }
