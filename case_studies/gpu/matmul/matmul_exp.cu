...
const int tm = 8;
 ...
 __global__ void __kernel0 (float* b_gmem, float* a_gmem, float* c_gmem, int p, int n, int m
)  {
  const int __ctx_sz = MSIZE2(exact_div(m, 32), exact_div(n, 32)) * MSIZE2(exact_div(32, 8), exact_div(32, 4));
  const int __tid = blockIdx.x * MSIZE2(exact_div(32, 8), exact_div(32, 4)) + threadIdx.x;
  SharedMemory smem;
  ...
  float* const sum = __treg_ref_uninit2<float>(8, 4);
   ...
   for (int bkIdx = 0; bkIdx < exact_div(p, 4); bkIdx++) {
     for (int k = 0; k < 4; k++) {
      a_smem[MINDEX4(exact_div(m, 32) * (exact_div(n, 32)), 4, 4, 8, 0, __ti4, k, __i5)] = a_gmem[MINDEX2(m, p, __bi0 * 32 + (
        __ti4 * 8 + __i5), bkIdx * 4 + k)];
    }
     for (int j = 0; j < 4; j++) {
      b_smem[MINDEX4(exact_div(m, 32) * (exact_div(n, 32)), 8, 4, 4, 0, __tj6, __k7, j)] = b_gmem[MINDEX2(p, n, bkIdx * 4 + __k7, __bj1 * 32 + (
        __tj6 * 4 + j))];
    }
    __syncthreads();
     ...
     for (int k = 0; k < 4; k++) {
      float* const a_regs = __treg_ref_uninit1_s<float>(8);
       for (int i = 0; i < 8; i++) {
        a_regs[MINDEX1(8, i)] = a_smem[MINDEX4(exact_div(m, 32) * (exact_div(n, 32)
        ), 4, 4, 8, 0, __ti8, k, i)];
      }
      float* const b_regs = __treg_ref_uninit1_s<float>(4);
       for (int j = 0; j < 4; j++) {
        b_regs[MINDEX1(4, j)] = b_smem[MINDEX4(exact_div(m, 32) * (exact_div(n, 32)
        ), 8, 4, 4, 0, __tj9, k, j)];
      }
       for (int i = 0; i < 8; i++) {
         for (int j = 0; j < 4; j++) {
          sum[MINDEX3(4 * 8, 8, 4, 0, i, j)] = sum[MINDEX3(4 * 8, 8, 4, 0, i, j)] + a_regs[MINDEX1(8, i)] * b_regs[MINDEX1(4, j)];
        }
      }
    }
     for (int i = 0; i < 8; i++) {  for (int j = 0; j < 4; j++) {  } }
   ...
   for (int i = 0; i < 8; i++) {
     for (int j = 0; j < 4; j++) {
      c_gmem[MINDEX2(m, n, __bi0 * 32 + (__ti10 * 8 + i), __bj1 * 32 + (__tj11 * 4 + j
  ...
  __kernel0<<<MSIZE2(exact_div(m, 32), exact_div(n, 32)), MSIZE2(exact_div(32, 8), exact_div(32, 4)), sizeof(float) * (
    4 * 4 * 8) + (sizeof(float) * (8 * 4 * 4) + 0)>>>(b_gmem, a_gmem, c_gmem, p, n, m);
  gmem_free(b_gmem);
  gmem_free(a_gmem);
