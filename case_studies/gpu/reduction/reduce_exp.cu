...
__global__ void __kernel0(float* d_partial_sums, float* d_arr, int N) /*@*/ {
  ...
  for (int i = 0; i < 2; i++) {
    tile[MINDEX2(exact_div(N, 512), 256, 0, __ti1)] =
        tile[MINDEX2(exact_div(N, 512), 256, 0, __ti1)] +
        d_arr[MINDEX1(N, __bi0 * 512 + (__ti1 * 2 + i))];
  }
  __syncthreads();
  float* const reduce_arr_12 = &tile[MINDEX2(exact_div(N, 512), 256, 0, 0)];
  for (int t = 0; t < 1 << 8; t++) {
  }
  for (int i = 8; i > 0; i--) {
    const int ei = 1 << i - 1;
    const int eii = 1 << i;
    if (__t2 < 1 << i - 1) {
      reduce_arr_12[MINDEX1(1 << 8, __t2)] =
          reduce_arr_12[MINDEX1(1 << 8, __t2)] +
          (&reduce_arr_12[ei])[MINDEX1(ei, __t2)];
    } else {
    }
    __syncthreads();
  }
  if (__ti_f3 == 0) {
    const float sum_temp_13 = reduce_arr_12[MINDEX1(1 << 8, 0)];
    for (int t = 0; t < 1 << 8; t++) {
    }
    d_partial_sums[MINDEX1(exact_div(N, 512), __bi0)] = sum_temp_13;
  } else {
  }
} /*@*/
...
float reduce(float* arr, int N) {
  ...
  __kernel0<<<MSIZE1(exact_div(N, 512)), MSIZE1(256),
              sizeof(float) * 256 + 0>>>(d_partial_sums, d_arr, N);
