...
let _ = Run.script_cpp (fun () ->
  ...
  !! Sequence.intro_on_instr ~mark:"bt_loops" [cFunDef "mm"; occFirst; cFor "j"];
  !! Sequence.intro_on_instr ~mark:"sum_loops" [cFunDef "mm"; occFirst; cFor "i"];
  let bt_loops = [cFunDef "mm"; cMark "bt_loops"] in
  let sum_loops = [cFunDef "mm"; cMark "sum_loops"] in

  bigstep "---- split i/j/k loops ----";

  !! Loop.tile (trm_int 32) ~index:"bj" ~bound:TileBoundDivides (bt_loops @ [cFor "j"]);
