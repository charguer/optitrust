...
void mm1024(float* C, float* A, float* B) {
  float* pB = (float*)malloc(
      sizeof(float[exact_div(1024, 32)][exact_div(1024, 4)][4][32]));
#pragma omp parallel for
  for (int bj = 0; bj < exact_div(1024, 32); bj++) {
    for (int bk = 0; bk < exact_div(1024, 4); bk++) {
      for (int k = 0; k < 4; k++) {
        for (int j = 0; j < 32; j++) {
...
#pragma omp parallel for
  for (int bi = 0; bi < exact_div(1024, 32); bi++) {
    for (int bj = 0; bj < exact_div(1024, 32); bj++) {
