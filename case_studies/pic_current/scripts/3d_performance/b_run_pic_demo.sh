...
run_gcc() {
  ...
  mpirun --report-bindings --cpus-per-proc $nb_threads -np $nb_sockets ./pic_demo.out ./parameters_3d.txt | tee ./std_output_run$1.txt
}
...
run_icc() {
  ...
  cd $PICVERT_HOME/3d_runs/run$1
  export OMP_NUM_THREADS=$nb_threads
  mpirun --report-bindings --cpus-per-proc $nb_threads -np $nb_sockets ./pic_demo.out ./parameters_3d | tee ./std_output_run$1.txt
}
