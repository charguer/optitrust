...
#include "math_functions.h" // function max

#define ROW_MAJOR      0
#define TILE           1
#define TILE_PRECALC   2
#define MORTON         3
#define MORTON_PRECALC 4
#define HILBERT        5
#define NB_CURVES      6
...
#  define COMPUTE_I_CELL_2D(num_cells_per_block_column, i_cell_x, i_cell_y) \
    (TILE_SIZE * (i_cell_x) + ((i_cell_y) & TILE_SIZE_MINUS_ONE) + num_cells_per_block_column * ((i_cell_y) / TILE_SIZE))
...
static inline int I_CELL_PARAM2_3D(int ncx, int ncy, int ncz) { return ncz * TILE_SIZE * TILE_SIZE * (ncx / TILE_SIZE); }
...
#  define ICX_FROM_I_CELL_2D(num_cells_per_block_column_minus_one, i_cell) ((i_cell & num_cells_per_block_column_minus_one) / TILE_SIZE)
...
#elif (I_CELL_2D_TYPE == TILE) || (I_CELL_2D_TYPE == TILE_PRECALC) // Tile
#  define ICY_FROM_I_CELL_2D(num_cells_per_block_column, i_cell) ((i_cell & TILE_SIZE_MINUS_ONE) + TILE_SIZE * (i_cell / num_cells_per_block_column))
...
static inline int ICX_PARAM2_3D(int ncx, int ncy, int ncz)                              { return ncz * TILE_SIZE * TILE_SIZE * (ncx / TILE_SIZE) - 1; }
#elif (I_CELL_3D_TYPE == MORTON) // Morton (no lookup table)
...
static inline int ICY_PARAM1_3D(int ncx, int ncy, int ncz)           { return ncz * TILE_SIZE * TILE_SIZE * (ncx / TILE_SIZE); }
...
#  elif (I_CELL_2D_TYPE == TILE) || (I_CELL_2D_TYPE == TILE_PRECALC) // Tile
static void REVERSE_I_CELL_2D(int num_cells_per_block_column, int i_cell, short int* icx, short int* icy) {
    *icx = (i_cell % num_cells_per_block_column) / TILE_SIZE;
    *icy = (i_cell % TILE_SIZE) + TILE_SIZE * (i_cell / num_cells_per_block_column);
}
#  elif (I_CELL_2D_TYPE == MORTON) // Morton (no lookup table)
