...
float vect_vect_mul(float* a, float* b, int n) {
  ...
  for (int bi = 0; bi < exact_div(n, 32); bi++) {
    ...
    __ghost(rewrite_linear,
            "inside := fun (i: int) -> &s ~~> reduce_sum(i, fun j -> A(j) *. "
            "B(j)), by := plus_zero_intro(bi * 32)");
    float t = 0.f;  // UPDATED
    __ghost(rewrite_float_linear_admitted, // ADDED
            "inside := fun v -> &t ~~> v, "
            " from := 0.f,"
            " to := reduce_sum(bi * 32 + 0, fun j -> A(j) *. B(j)) -. reduce_sum(bi * 32, fun j -> A(j) *. B(j))");
    for (int i = 0; i < 32; i++) {
      __strict();
      ...
      t = t + a[MINDEX1(n, bi * 32 + i)] * b[MINDEX1(n, bi * 32 + i)]; // UPDATED, with arith simpl
    ...
    __ghost(rewrite_float_linear_admitted, // ADDED
              "inside := fun v -> &s ~~> v, "
              " from := reduce_sum(bi * 32 + 32, fun j -> A(j) *. B(j)) -. reduce_sum(bi * 32, fun j -> A(j) *. B(j)) +. reduce_sum(bi * 32, fun j -> A(j) *. B(j)),"
              " to := reduce_sum(bi * 32 + 32, fun j -> A(j) *. B(j))");
    __ghost(rewrite_linear,
            "inside := fun (i: int) -> &s ~~> reduce_sum(i, fun j -> A(j) *. "
            "B(j)), by := mul_add_factor(bi, 32)");
  }
  __ghost(rewrite_linear,
          "inside := fun (i: int) -> &s ~~> reduce_sum(i, fun j -> A(j) *. "
