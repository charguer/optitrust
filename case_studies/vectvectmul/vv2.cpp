...
float vect_vect_mul(float* a, float* b, int n) {
  ...
  for (int bi = 0; bi < exact_div(n, 32); bi++) {
    ...
    float t = 0.f;
    // TODO: needs to be introduced by arith simpl
    __ghost([&] {
      __consumes("&t ~~> 0.f");
      __produces("&t ~~> reduce_sum(bi * 32, fun j -> A(j) *. B(j)) -. reduce_sum(bi * 32, fun j -> A(j) *. B(j))");
      __admitted();
    });
    __ghost(rewrite_linear,
            "inside := fun (i: int) -> &t ~~> reduce_sum(i, fun j -> A(j) *. "
            "B(j)) -. reduce_sum(bi * 32, fun j -> A(j) *. B(j)), by := "
            "plus_zero_intro(bi * 32)");
    for (int i = 0; i < 32; i++) {
      __strict();
      __spreserves("&s ~~> reduce_sum(bi * 32, fun j -> A(j) *. B(j))");
      __spreserves(
          "&t ~~> reduce_sum(bi * 32 + i, fun j -> A(j) *. B(j)) -. "
          "reduce_sum(bi * 32, fun j -> A(j) *. B(j))");
      __sreads("a ~> Matrix1(n, A)");
      __sreads("b ~> Matrix1(n, B)");
      ...
      t = t + a[MINDEX1(n, bi * 32 + i)] * b[MINDEX1(n, bi * 32 + i)];
      // TODO: needs to be introduced by arith simpl
      __ghost([&] {
        __consumes("&t ~~> reduce_sum(bi * 32 + i, fun j -> A(j) *. B(j)) -. reduce_sum(bi * 32, fun j -> A(j) *. B(j)) +. A(bi * 32 + i) *. B(bi * 32 + i)");
        __produces("&t ~~> reduce_sum(bi * 32 + i, fun j -> A(j) *. B(j)) -. reduce_sum(bi * 32, fun j -> A(j) *. B(j)) +. reduce_sum(bi * 32, fun j -> A(j) *. B(j)) +. A(bi * 32 + i) *. B(bi * 32 + i) -. reduce_sum(bi * 32, fun j -> A(j) *. B(j))");
        __admitted();
      });
      __ghost(rewrite_float_linear,
              "inside := fun (v: float) -> &t ~~> v +. A(bi * 32 + i) *. B(bi * "
              "32 + i) -. reduce_sum(bi * 32, fun j -> A(j) *. B(j)), by := "
              "r_cancel_minus_plus(reduce_sum(bi * 32 + i, fun j -> A(j) *. "
              "B(j)), reduce_sum(bi * 32, fun j -> A(j) *. B(j)))");
      __ghost_end(focusA);
      __ghost_end(focusB);
      __ghost(in_range_bounds, "x := bi * 32 + i", "i_gt_0 <- lower_bound");
      __ghost(rewrite_float_linear,
              "inside := fun v -> &t ~~> v -. reduce_sum(bi * 32, fun j -> A(j) "
              "*. B(j)), by := reduce_sum_add_right(bi * 32 + i, fun j -> A(j) "
              "*. B(j), i_gt_0)");
      __ghost(rewrite_linear,
              "inside := fun (i: int) -> &t ~~> reduce_sum(i, fun j -> A(j) *. "
              "B(j)) -. reduce_sum(bi * 32, fun j -> A(j) *. B(j)), by := "
              "add_assoc_right(bi * 32, i, 1)");
    }
    __ghost(rewrite_linear,
            "inside := fun (i: int) -> &t ~~> reduce_sum(i, fun j -> A(j) *. "
            "B(j)) -. reduce_sum(bi * 32, fun j -> A(j) *. B(j)), by := "
            "mul_add_factor(bi, 32)");
    s = t + s;
    // NOTE: inlined block expression
    __ghost(rewrite_float_linear,
            "inside := fun (v: float) -> &s ~~> v, by := "
            "r_cancel_minus_plus(reduce_sum((bi + 1) * 32, fun j -> A(j) *. "
            "B(j)), reduce_sum(bi * 32, fun j -> A(j) *. B(j)))");
  }
  __ghost(rewrite_linear,
          "inside := fun (i: int) -> &s ~~> reduce_sum(i, fun j -> A(j) *. "
