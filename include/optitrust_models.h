...
__GHOST(ro_matrix1_focus) {
  ...
  __produces("Wand(_RO(f, &matrix[MINDEX1(n, i)] ~~>[MT] M(i)), _RO(f, matrix ~> Matrix1Of(n, MT, M))), _RO(f, &matrix[MINDEX1(n, i)] ~~>[MT] M(i))");
...
__GHOST(ro_matrix2_focus) {
  ...
  __produces("Wand(_RO(f, &matrix[MINDEX2(m, n, i, j)] ~~>[MT] M(i, j)), _RO(f, matrix ~> Matrix2Of(m, n, MT, M))), _RO(f, &matrix[MINDEX2(m, n, i, j)] ~~>[MT] M(i, j))");
  __admitted(); // for efficiency
...
__GHOST(matrix1_span_shift) {
  __requires("T: Type, matrix: ptr(T), n1: int, a: int, b: int, M: int -> T");
  __consumes("for i in a..b -> &matrix[MINDEX1(n1, i)] ~~> M(i)");
  __produces("for i in 0..(b-a) -> &(&matrix[a])[MINDEX1(b-a, i)] ~~> M(i+a)");
  __admitted();
}
