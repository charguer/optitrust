...
(** [normalize_trm t evar_ctx]: tries to normalize [t], aiming to match equivalent trms with different syntax
Performs the following normalisations:
  - normalize mindex : &(&x[MINDEX_(dims_in,inds_in,0,..,0)])[MINDEX_(dims_out,inds_out)] -->  &x[MINDEX_(dims_in,inds_in,inds_out)]
If normalization is not possible, return the same [t]
*)
let rec normalize_trm ?(on_failure = fun a b -> ()) (t:trm) (evar_ctx: 'a unification_ctx ) (validate_inst: trm -> 'a -> 'a unification_ctx -> 'a unification_ctx option) : trm * 'a unification_ctx =
    ...
    |Some (base_in,dims_in,inds_in) ->
      let base_in, evar_ctx =  aux base_in evar_ctx in
        ...
        if List.length dims_in >= n_dims_out then begin
          ...
          let unify_result = begin
            try
              List.fold_left2 (fun evar_ctx arg arge -> let* evar_ctx in trm_unify ~on_failure arg arge evar_ctx validate_inst) (Some evar_ctx) left_list right_list
...
(** [trm_unify t1 t2 evar_ctx validate_inst] tries to unify [t1] with [t2],
    ...
    *)
and trm_unify ?(on_failure = fun a b -> ()) (t_left: trm) (t_right: trm) (evar_ctx: 'a unification_ctx) (validate_inst: trm -> 'a -> 'a unification_ctx -> 'a unification_ctx option) : 'a unification_ctx option =
 ...
 let open Option.Monad in
  ...
  | Trm_apps (f, args, [], []), Trm_apps (fe, argse, [], []) ->
    let res_opt =
      ...
      with Invalid_argument _ -> None end (* todo: better to check lengths first *)
    in
