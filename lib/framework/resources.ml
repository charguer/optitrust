...
let%transfo perform_elaboration (_u: unit): unit =
  Trace.recompute_resources ~elaboration:true ~missing_types:false ();
  (* let rec replace_to_elaborate (t: trm) =
    match t.desc with
     ...TOFIX
  ...
  in
  Trace.apply replace_to_elaborate; *)
  Trace.justif "Inferring contents for elaboration holes"

let%transfo fix_types_in_contracts (_u: unit): unit =
  Trace.recompute_resources ~elaboration:false ~missing_types:true ();
