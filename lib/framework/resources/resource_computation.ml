...
let find_fun_spec (t: trm) (fun_specs: fun_spec_resource varmap): fun_spec_resource =
  ...
  | Some fn ->
    Option.unsome_or_else (Var_map.find_opt fn fun_specs) (fun () -> raise (Spec_not_found fn))
  | None ->
...
let formula_style = C_encoding.style_of_custom_style (Style.default_custom_style ())
...
let rec compute_resources
  ...
  let usage_map, res =
      ...
      | spec ->
        ...
        *)
          ...
          List.fold_left2 (fun acc contract_arg effective_arg ->
            ...
            let subst_map = if Var_set.mem contract_arg contract_fv then begin
              let arg_formula = match formula_of_trm effective_arg with
                ...
                | None -> trm_fail effective_arg (Printf.sprintf "Could not make a formula out of term '%s', required because of instantiation of %s contract" (Ast_to_c.ast_to_string effective_arg) (Ast_to_c.ast_to_string fn))
              in
        ...
        with Invalid_argument _ ->
          failwith "Mismatching number of arguments for %s" (Ast_to_c.ast_to_string fn)
        in
    ...
    | _ -> trm_fail t ("Resource_core.compute_inplace: not implemented for " ^ Ast_to_c.ast_to_string t)
    end with
