...
let rec compute_pure_typ (env: pure_env) ?(typ_hint: typ option) (t: trm): typ =
  ...
  | Trm_apps (f, args, gargs, gbind) ->
    ...
    | Trm_prim (typ, prim), (first_arg :: _) when !missing_types_in_contracts && is_typ_auto typ ->
      let first_arg_typ = compute_pure_typ env first_arg in
      compute_pure_typ env (trm_apps (trm_prim first_arg_typ prim) args)
