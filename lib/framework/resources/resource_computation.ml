...
let rec compute_pure_typ (env: pure_env) ?(typ_hint: typ option) (t: trm): typ =
  ...
  | Trm_apps (f, args, gargs, gbind) ->
    ...
    | Trm_var xf, [ptr; repr] when var_eq xf var_repr ->
      ...
      let mem_typ_check mem_typ_var (cont: unit -> unit) =
        let mem_typ = compute_pure_typ env mem_typ_var in
        if not (Trm_unify.are_same_trm mem_typ typ_mem_type) then
          failwith "In '%s': hardware type '%s' is not recognized" (Ast_to_c.ast_to_string t) (Ast_to_c.ast_to_string mem_typ_var)
        else cont () in
