...
let with_captured_show ?(activated:bool=true) (dest : string ref) (f : unit -> 'a) : 'a =
  if not activated then f() else begin
    ...
    let finalize () =
      close_out c;
      dest := File.get_contents tmp_file;
      prt_channel := saved
      in
    try f(); finalize()
...
let typ ?(msg : string = "") (t : typ) : unit =
  let t_str = Ast_to_c.typ_to_string t in
  prt_msg msg;
  prt ~suffix:"\n" t_str
    ...
    if not internal then style_default else begin
      ...
      { style_default with
        ...
        print = Lang_C { cstyle with
          ...
          print_contract_internal_repr = Option.value ~default:cstyle.print_contract_internal_repr contract_internal_repr;
          print_var_id = Option.value ~default:cstyle.print_var_id var_id;
        }
      }
    end in
