...
(** Printer for the OptiLambda textual language.
    ...
    This file intentionally depends only on [optitrust.ast], [optitrust.utils],
    and [pprint]. It must not depend on [optitrust.framework], because the
    framework trace/diff layer should later be able to depend on this module. *)
...
let var_to_doc (style : Optilambda_style.style) (v : var) : document =
  string (if style.print_var_ids then var_to_string v else var_name v)
...
let rec typ_to_doc (style : Optilambda_style.style) (ty : typ) : document =
  ...
  | Trm_apps ({ desc = Trm_var v; _ }, args, [], []) ->
    ...
    | "ptr", [inner] ->
      string "ptr" ^^ parens_doc (typ_to_doc style inner)
    | "const", [inner] ->
      string "const" ^^ parens_doc (typ_to_doc style inner)
    | "array", [inner] ->
      string "array" ^^ parens_doc (typ_to_doc style inner)
    | "array", [inner; size] ->
      string "array" ^^ parens_doc (comma_sep [typ_to_doc style inner; trm_to_doc_at style 0 size])
    | "fun", ret :: args ->
      string "fun" ^^ parens_doc (comma_sep (typ_to_doc style ret :: List.map (typ_to_doc style) args))
    | _ ->
      var_to_doc style v ^^ parens_doc (comma_sep (List.map (trm_to_doc_at style 0) args))
    end
  | Trm_arbitrary (Typ code) ->
    string code
  | _ ->
    string "type" ^^ parens_doc (trm_to_doc_at style 0 ty)
...
and typed_var_to_doc (style : Optilambda_style.style) ((v, ty) : typed_var) : document =
  if style.print_types && not (is_auto_type ty)
  then var_to_doc style v ^^ colon ^^ blank 1 ^^ typ_to_doc style ty
...
and lit_to_doc (style : Optilambda_style.style) (lit : lit) : document =
  ...
  | Lit_null ty ->
    string "null" ^^ angles_doc (typ_to_doc style ty)
...
and binop_to_doc (op : binary_op) : document option =
  ...
  | Binop_exact_div | Binop_trunc_div -> Some (string "/")
  ...
  | Binop_array_access | Binop_array_get -> None

(** [binop_precedence op] gives larger numbers to more tightly binding operators. *)
and binop_precedence (op : binary_op) : int =
  match op with
  | Binop_set -> 1
  | Binop_eq | Binop_neq | Binop_le | Binop_lt | Binop_ge | Binop_gt -> 3
  | Binop_bitwise_or | Binop_xor -> 4
  | Binop_bitwise_and | Binop_shiftl | Binop_shiftr -> 5
  | Binop_add | Binop_sub -> 6
  | Binop_mul | Binop_exact_div | Binop_trunc_div | Binop_trunc_mod -> 7
  | Binop_array_access | Binop_array_get -> 10

(** [trm_precedence t] gives the precedence of [t] when printed as an expression. *)
and trm_precedence (t : trm) : int =
  match t.desc with
  | Trm_var _ | Trm_lit _ | Trm_prim _ | Trm_arbitrary _ -> 11
  | Trm_apps ({ desc = Trm_prim (_, Prim_binop op); _ }, _, _, _) ->
    binop_precedence op
  | Trm_apps ({ desc = Trm_prim (_, Prim_unop _); _ }, _, _, _) ->
    8
  | Trm_apps _ ->
    10
  | _ ->
...
and prim_to_doc (style : Optilambda_style.style) (ty : typ) (prim : prim) : document =
  match prim with
  | Prim_unop (Unop_cast cast_ty) ->
    string "cast" ^^ angles_doc (typ_to_doc style cast_ty)
  | Prim_unop op ->
    string (Option.value ~default:"unop" (unop_name op))
  | Prim_binop op ->
...
and ghost_args_to_doc (style : Optilambda_style.style) (ghost_args : resource_item list) : document =
  brackets_doc
    (comma_sep
       (List.map
          (fun (hyp, formula) ->
            var_to_doc style hyp ^^ blank 1 ^^ string ":=" ^^ blank 1 ^^ trm_to_doc_at style 0 formula)
          ghost_args))
...
and call_suffix_to_doc (style : Optilambda_style.style) ghost_args ghost_bind : document =
  let ghost_args_doc =
    if ghost_args = [] then empty else ghost_args_to_doc style ghost_args
  in
  let ghost_bind_doc =
    if ghost_bind = [] then empty else ghost_bind_to_doc style ghost_bind
  in
...
and trm_has_attribute (attr : attribute) (t : trm) : bool =
  List.exists (( = ) attr) t.annot.trm_annot_attributes
...
(** [contract_clause_to_doc style keyword item] prints one contract clause line. *)
and contract_clause_to_doc (style : Optilambda_style.style) (keyword : string) (item : resource_item) : document =
  string keyword ^^ blank 1 ^^ resource_item_to_doc style item

(** [contract_clauses_to_doc style keyword items] prints a group of contract clauses. *)
and contract_clauses_to_doc (style : Optilambda_style.style) (keyword : string) (items : resource_item list) : document list =
  List.map (contract_clause_to_doc style keyword) items

(** [fun_contract_clause_docs style contract] prints the direct internal function contract. *)
and fun_contract_clause_docs (style : Optilambda_style.style) (contract : fun_contract) : document list =
  if not style.print_contracts then [] else
  contract_clauses_to_doc style "requires" contract.pre.pure
  @ contract_clauses_to_doc style "consumes" contract.pre.linear
  @ contract_clauses_to_doc style "ensures" contract.post.pure
  @ contract_clauses_to_doc style "produces" contract.post.linear

(** [fun_spec_clause_docs style spec] prints clauses carried by a function spec. *)
and fun_spec_clause_docs (style : Optilambda_style.style) (spec : fun_spec) : document list =
  match spec with
  | FunSpecUnknown -> []
  | FunSpecContract contract -> fun_contract_clause_docs style contract
  | FunSpecReverts fn -> if style.print_contracts then [string "reverts" ^^ blank 1 ^^ var_to_doc style fn] else []

(** [loop_contract_clause_docs style contract] prints the direct internal loop contract. *)
and loop_contract_clause_docs (style : Optilambda_style.style) (contract : loop_contract) : document list =
  if not style.print_contracts then [] else
  let strict_doc = if contract.strict then [string "strict"] else [] in
  strict_doc
  @ contract_clauses_to_doc style "requires" contract.loop_ghosts
  @ contract_clauses_to_doc style "requires" contract.invariant.pure
  @ contract_clauses_to_doc style "preserves" contract.invariant.linear
  @ contract_clauses_to_doc style "reads" contract.parallel_reads
  @ contract_clauses_to_doc style "xrequires" contract.iter_contract.pre.pure
  @ contract_clauses_to_doc style "xconsumes" contract.iter_contract.pre.linear
  @ contract_clauses_to_doc style "xensures" contract.iter_contract.post.pure
  @ contract_clauses_to_doc style "xproduces" contract.iter_contract.post.linear

(** [fun_spec_items spec] collects resources mentioned by a function spec. *)
and fun_spec_items (spec : fun_spec) : resource_item list =
  match spec with
  | FunSpecUnknown | FunSpecReverts _ -> []
  | FunSpecContract contract ->
    contract.pre.pure @ contract.pre.linear @ contract.post.pure @ contract.post.linear
...
and loop_contract_items (contract : loop_contract) : resource_item list =
  ...
  @ contract.invariant.pure
  @ contract.invariant.linear
  @ contract.parallel_reads
...
and contract_summary_to_doc (style : Optilambda_style.style) (items : resource_item list) : document =
  if not style.print_contracts || items = [] then empty else
    blank 1 ^^ brackets_doc (comma_sep (List.map (fun (hyp, _) -> var_to_doc style hyp) items))
...
and trm_to_block_doc_with_prefix (style : Optilambda_style.style) (prefix_docs : document list) (t : trm) : document =
  ...
  | Trm_seq (instrs, result) ->
    let instr_docs = List.map (trm_to_doc_at style 0) (Mlist.to_list instrs) in
    let result_docs =
      match result with
      | None -> []
      | Some v -> [string "return" ^^ blank 1 ^^ var_to_doc style v]
    in
    block_doc (prefix_docs @ instr_docs @ result_docs)
  | _ ->
    block_doc (prefix_docs @ [trm_to_doc_at style 0 t])

(** [fun_def_to_doc style ?type_params name args ret_ty body] prints a function definition. *)
and fun_def_to_doc
    (style : Optilambda_style.style)
    ?(type_params = [])
    (name : var option)
    (args : typed_vars)
    (ret_ty : typ)
  ...
  let ret_doc =
    if style.print_types && not (is_auto_type ret_ty)
    then colon ^^ blank 1 ^^ typ_to_doc style ret_ty
    else empty
  in
  let contract_docs = fun_spec_clause_docs style spec in
  group
    (string "fun" ^^ name_doc ^^ type_params_doc ^^ args_doc ^^ ret_doc
     ^^ contract_summary_to_doc style (fun_spec_items spec)
     ^^ blank 1
     ^^ trm_to_block_doc_with_prefix style contract_docs body)
...
and let_to_doc (style : Optilambda_style.style) ((v, ty) : typed_var) (body : trm) : document =
  ...
  | Trm_fun (args, ret_ty, fun_body, spec) ->
    fun_def_to_doc style (Some v) args ret_ty spec fun_body
  ...
  | Trm_apps ({ desc = Trm_prim (_, Prim_ref_uninit); _ }, [], [], []) ->
    string "letmut" ^^ blank 1 ^^ var_to_doc style v
  | _ ->
    let typed_doc =
      if style.print_types && not (is_auto_type ty)
      then var_to_doc style v ^^ colon ^^ blank 1 ^^ typ_to_doc style ty
...
and app_to_doc
  ...
  | Trm_prim (_, Prim_binop op), [lhs; rhs] ->
    ...
    | None ->
      prim_to_doc style typ_auto (Prim_binop op) ^^ parens_doc (comma_sep (List.map (trm_to_doc_at style 0) args))
    end
  | Trm_prim (_, Prim_unop (Unop_struct_get field)), [base] ->
    trm_to_doc_at style 10 base ^^ string "." ^^ string field
  | Trm_prim (_, Prim_unop (Unop_struct_access field)), [base] ->
    trm_to_doc_at style 10 base ^^ string "->" ^^ string field
  | Trm_prim (_, Prim_unop Unop_get), [arg] ->
    trm_to_doc_at style 8 arg
  | Trm_prim (_, Prim_unop Unop_address), [arg] ->
    string "&" ^^ trm_to_doc_at style 8 arg
  | Trm_prim (_, Prim_unop Unop_minus), [arg] ->
    string "-" ^^ trm_to_doc_at style 8 arg
  | Trm_prim (_, Prim_unop Unop_neg), [arg] ->
    string "not" ^^ blank 1 ^^ trm_to_doc_at style 8 arg
  ...
  | Trm_prim (ty, prim), _ ->
    prim_to_doc style ty prim ^^ parens_doc (comma_sep (List.map (trm_to_doc_at style 0) args))
  | _ ->
...
and ghost_to_doc (style : Optilambda_style.style) (t : trm) : document option =
  if not style.print_ghosts || not (trm_has_attribute GhostInstr t) then None else
  match t.desc with
  | Trm_let ((pair_var, _), { desc = Trm_apps (ghost_begin, [{ desc = Trm_apps (f, args, ghost_args, ghost_bind); _ }], [], []); _ })
    ...
    Some
      (string "ghost_begin"
       ^^ parens_doc
            (comma_sep [var_to_doc style pair_var; ghost_call_to_doc style f args ghost_args ghost_bind]))
  | Trm_apps (ghost_end, [{ desc = Trm_var pair_var; _ }], [], [])
      when var_has_name "__ghost_end" ghost_end ->
...
and seq_to_doc (style : Optilambda_style.style) (instrs : trm mlist) (result : var option) : document =
  ...
  let result_docs =
    ...
    | Some v -> [string "return" ^^ blank 1 ^^ var_to_doc style v]
  in
  block_doc (instr_docs @ result_docs)
...
and for_to_doc
    ...
    : document =
  let mode_docs =
    if style.print_loop_direction
    then [loop_mode_to_doc style mode; loop_dir_to_doc range.direction]
    else [loop_mode_to_doc style mode]
  in
  let step_docs =
    if style.omit_default_loop_step && is_int_one range.step
    then []
    else [trm_to_doc_at style 0 range.step]
  in
  let range_doc =
    parens_doc
      (comma_sep
         ((var_to_doc style range.index ^^ blank 1 ^^ equals ^^ blank 1 ^^ trm_to_doc_at style 0 range.start)
          :: trm_to_doc_at style 0 range.stop
          :: step_docs))
  in
  string "for"
  ^^ angles_doc (comma_sep mode_docs)
  ^^ range_doc
  ^^ contract_summary_to_doc style (loop_contract_items contract)
  ^^ blank 1
  ^^ trm_to_block_doc_with_prefix style (loop_contract_clause_docs style contract) body
...
and record_member_to_doc (style : Optilambda_style.style) ((member, _) : record_member * record_member_annot) : document =
  match member with
  | Record_field (label, ty) ->
    string label ^^ colon ^^ blank 1 ^^ typ_to_doc style ty
  | Record_method method_trm ->
    trm_to_doc_at style 0 method_trm
...
and typedef_to_doc (style : Optilambda_style.style) (t : trm) : document =
  ...
  | Trm_typedef td ->
    ...
    | Typedef_alias ty ->
      string "type" ^^ blank 1 ^^ name_doc ^^ blank 1 ^^ equals ^^ blank 1 ^^ typ_to_doc style ty
...
and switch_to_doc (style : Optilambda_style.style) (cond : trm) (cases : (trms * trm) list) : document =
  ...
  in
  string "switch" ^^ blank 1 ^^ parens_doc (trm_to_doc_at style 0 cond) ^^ blank 1
  ^^ block_doc (List.map case_to_doc cases)

(** [marks_to_doc marks] prints term marks as an OptiLambda attribute prefix. *)
and marks_to_doc (marks : marks) : document =
  string "@marks" ^^ brackets_doc (comma_sep (List.map string marks))
...
and add_marks_to_doc (style : Optilambda_style.style) (t : trm) (doc : document) : document =
  if style.print_marks && t.annot.trm_annot_marks <> []
  then marks_to_doc t.annot.trm_annot_marks ^^ blank 1 ^^ doc
  else doc
...
and trm_to_doc_at (style : Optilambda_style.style) (ctx_prec : int) (t : trm) : document =
  ...
  | Trm_let_mult bindings ->
    block_doc (List.map (fun (typed_var, body) -> let_to_doc style typed_var body) bindings)
  | Trm_predecl typed_var ->
    string "let" ^^ blank 1 ^^ typed_var_to_doc style typed_var
  | Trm_fun (args, ret_ty, body, spec) ->
    fun_def_to_doc style None args ret_ty spec body
  | Trm_typedef _ ->
    typedef_to_doc style t
  | Trm_if (cond, then_branch, else_branch) ->
    string "if"
    ^^ blank 1
    ^^ parens_doc (trm_to_doc_at style 0 cond)
    ^^ blank 1
    ^^ trm_to_block_doc style then_branch
    ^^ blank 1
    ^^ string "else"
    ^^ blank 1
    ^^ trm_to_block_doc style else_branch
  | Trm_seq (instrs, result) ->
    seq_to_doc style instrs result
  | Trm_apps (f, args, ghost_args, ghost_bind) ->
    app_to_doc style f args ghost_args ghost_bind
  | Trm_for (range, mode, body, contract) ->
    for_to_doc style range mode contract body
  | Trm_for_c (init, cond, step, body, _) ->
    ...
    ^^ blank 1
    ^^ trm_to_block_doc style body
  | Trm_while (cond, body) ->
    string "while" ^^ blank 1 ^^ parens_doc (trm_to_doc_at style 0 cond) ^^ blank 1 ^^ trm_to_block_doc style body
  | Trm_do_while (body, cond) ->
    string "do" ^^ blank 1 ^^ trm_to_block_doc style body ^^ blank 1 ^^ string "while" ^^ blank 1 ^^ parens_doc (trm_to_doc_at style 0 cond)
  | Trm_switch (cond, cases) ->
    switch_to_doc style cond cases
  | Trm_abort abort ->
  ...
  | Trm_goto label ->
    string "goto" ^^ blank 1 ^^ string label
  | Trm_arbitrary code ->
    string (code_to_str code)
  | Trm_omp_routine _ ->
    string "omp_routine"
  | Trm_extern (lang, trms) ->
    string "extern" ^^ blank 1 ^^ dquotes (string lang) ^^ blank 1 ^^ block_doc (List.map (trm_to_doc_at style 0) trms)
  | Trm_namespace (name, body, _) ->
    string "namespace" ^^ blank 1 ^^ string name ^^ blank 1 ^^ trm_to_block_doc style body
  | Trm_template (params, body) ->
    ...
    | _ ->
      ...
      ^^ blank 1
      ^^ trm_to_doc_at style 0 body
    end
  | Trm_using_directive name ->
    string "using" ^^ blank 1 ^^ string name
  in
...
and trm_to_doc (style : Optilambda_style.style) (t : trm) : document =
  trm_to_doc_at style 0 t

(** [typ_to_string ?style ty] prints a type directly to a string. *)
let typ_to_string ?(style = Optilambda_style.default) (ty : typ) : string =
  Tools.document_to_string (typ_to_doc style ty)

(** [trm_to_string ?style t] prints a term directly to a string. *)
let trm_to_string ?(style = Optilambda_style.default) (t : trm) : string =
  Tools.document_to_string (trm_to_doc style t)
