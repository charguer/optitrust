...
let%transfo create_kernel_launch ?(grid_override: trm list option) ?(setup_end: target option) ?(teardown_begin: target option) (bpgs: trm list) (tpbs: trm list) (smem_szs: trm list)
  ...
  let insert_helper code tg =
    Target.apply_at_target_paths_before (fun t i -> Sequence_core.insert_at code i t) tg in
  (fun () -> (fun next_m ->
    ...
    insert_helper launch start;
    insert_helper setup (Option.unsome_or_else setup_end (fun () -> [tAfter; cMark launch_mark]));
    insert_helper kill stop;
    insert_helper teardown (Option.unsome_or_else teardown_begin (fun () -> [tBefore; cMark kill_mark]));
