    ...
    let rec hoist_aux name_template (i : int) =
      ...
      | 1 -> begin
        ...
        Trace.without_resource_computation_between_steps (fun () ->
          ...
          if inline then Trace.without_substep_validity_checks simpl_hoist_tmp_var (* (fun () -> Flags.with_flag Flags.use_resources_with_models false simpl_hoist_tmp_var); *)
        )
        );
      end
