...
let hoist_on (name : string)
  ...
  if contract.strict then
    ...
    ) (List.combine other_indices dims) Resource_formula.(formula_uninit_cell ~hw:Resource_formula.trm_any_ty access) in
...
let move_out_alloc_on (trm_index : int) (t : trm) : trm =
  ...
  let open Resource_formula in
  let contract = { contract with invariant = { contract.invariant with linear = (new_anon_hyp (), formula_uninit_matrix ~hw:Resource_formula.trm_any_ty (trm_var array_var) dims) :: contract.invariant.linear }} in
