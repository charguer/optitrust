...
let hoist_on (name : string)
  ...
  if contract.strict then
    ...
    ) (List.combine other_indices dims) Resource_formula.(formula_uninit_cell ~mem_typ:Resource_formula.mem_typ_any access) in
...
let move_out_alloc_on (trm_index : int) (t : trm) : trm =
  ...
  let open Resource_formula in
  let contract = { contract with invariant = { contract.invariant with linear = (new_anon_hyp (), formula_uninit_matrix ~mem_typ:Resource_formula.mem_typ_any (trm_var array_var) dims) :: contract.invariant.linear }} in
