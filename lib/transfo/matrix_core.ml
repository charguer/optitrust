...
let reorder_dims_aux ~(base : trm) ~(dims : trms) (rotate_n : int) (order : int list) (t : trm) : trm =
  ...
  | None -> match let_alloc_inv t with
      | Some (var, typ, dims, init) ->
        let before = Resource_formula.formula_matrix ~hw:Resource_formula.trm_any_ty ~init base reordered_dims in
        let after = Resource_formula.formula_reorder_dims_patch ~hw:Resource_formula.trm_any_ty ~init base dims order in
      ...
      | None -> match free_inv t with
        | Some var ->
          let before = Resource_formula.formula_reorder_dims_patch ~hw:Resource_formula.trm_any_ty ~init:false base dims order in
          let after = Resource_formula.formula_matrix ~hw:Resource_formula.trm_any_ty ~init:false base reordered_dims in
          Nobrace.trm_seq_nomarks
                [
                   Resource_trm.ghost_admitted_rewrite before after (trm_var (toplevel_var "reorder_groups"));
                  free var ]
