...
let split_fields_on (typvar : typvar) (field_list : (field * typ) list)
  ...
  update_span_helper span t_seq (fun span_instrs ->
    ...
    let process_matching_resource_item process_one_cell default (h, formula) =
      ...
      let rec aux wrap_cell formula =
        Pattern.pattern_match formula [
          ...
          Pattern.(formula_cell !__ !__) (fun loc hw () -> (* TODO should be restricted to any? *)
    ...
    let process_one_cell ~(fold : bool) wrap_cell (is_ro, loc) =
      let model loc = wrap_cell (formula_cell ~hw:Resource_formula.trm_any_ty loc) in
      if is_ro then begin
          ...
          process_matching_resource_item (fun wrap _mode loc ->
            ...
            List.map (fun (sf, ty) ->
              ...
              | Some { frac; formula } ->
                ...
                formula_read_only ~frac:new_frac (wrap (formula_cell ~hw:Resource_formula.trm_any_ty (trm_struct_access ~field_typ:ty ~struct_typ loc sf)))
              | None when is_formula_uninit formula -> wrap (formula_uninit_cell ~hw:Resource_formula.trm_any_ty (trm_struct_access ~field_typ:ty ~struct_typ loc sf))
              | None -> wrap (formula_cell ~hw:Resource_formula.trm_any_ty (trm_struct_access ~field_typ:ty ~struct_typ loc sf))
              in
