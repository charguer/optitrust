    ...
    let unfold_cells clause_locs =
      ...
      if !Flags.check_validity then begin
        ...
        let make_one_pair (clause, loc) =
          ...
          | Reads ->
            let per_field_admits = List.map (fun (sf, ty) ->
              let with_fresh_fracs () =
                ...
                let folded_res =
                  formula_read_only ~frac:(trm_var frac_var) (formula_cell ~hw:trm_any_ty loc)
                in
                let unfolded_res =
                  formula_read_only ~frac:(trm_var frac_var)
                    (formula_cell ~hw:trm_any_ty (trm_struct_access ~field_typ:ty ~struct_typ loc sf))
                in
          ...
          | Writes ->
            let folded_linear = [(
              new_anon_hyp (), formula_cell ~hw:trm_any_ty loc
            )] in
            let unfolded_linear = List.map (fun (sf, ty) ->
              (new_anon_hyp (), formula_cell ~hw:trm_any_ty (trm_struct_access ~field_typ:ty ~struct_typ loc sf))
            ) field_list in
  ...
  let aux_resource_items =
    ...
    List.concat_map (fun (h, r) ->
      ...
      Pattern.pattern_match inner_r [
        Pattern.(formula_cell (trm_specific_var var) !__) (fun hw () ->
          List.map (fun (f, f_typ, v) ->
            (new_anon_hyp (), maybe_ro (formula_cell ~hw (trm_var ~typ:(typ_ptr f_typ) v)))
          ) fields
        );
        Pattern.(formula_uninit_cell (trm_specific_var var) !__) (fun hw () ->
          List.map (fun (f, f_typ, v) ->
            (new_anon_hyp (), formula_uninit_cell ~hw (trm_var ~typ:(typ_ptr f_typ) v))
          ) fields
        );
        Pattern.__ (fun () -> [(h, r)])
      ]
