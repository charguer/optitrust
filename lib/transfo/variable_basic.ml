      ...
      step_backtrack ~discard_after:true (fun () ->
        ...
        Target.apply_at_path (fun t ->
          let (_, open_w, close_w) = Resource_trm.ghost_pair_hide
            (Resource_formula.formula_cell_var ~hw:Resource_formula.trm_any_ty ~typ:var_typ var) in
    ...
    if !Flags.check_validity then begin
      step_backtrack ~discard_after:true (fun () ->
        Target.apply_at_path (fun t_seq ->
          ...
          let y_cell = Resource_formula.formula_uninit_cell_var ~hw:Resource_formula.trm_any_ty y in
          ...
          let instrs, end_trms = match !copy_back_index with
            ...
            | Some i ->
              ...
              let x_cell = Resource_formula.formula_uninit_cell_var ~hw:Resource_formula.trm_any_ty x in
