...
(******************************************************************************)
(*              Resettable references and fresh name generation               *)
(******************************************************************************)

let reset_closures = ref []

let resetable_ref (init_val: 'a): 'a ref =
  let new_ref = ref init_val in
  let reset () = new_ref := init_val in
  reset_closures := reset :: !reset_closures;
  new_ref

(** [reset_all_refs]: reset all the references and generators created by this module *)
let reset_all_refs () =
  List.iter (fun reset -> reset ()) !reset_closures
...
let cart_prod op s1 s2 = List.concat_map (fun x1 -> List.map (fun x2 -> op x1 x2) s2) s1
...
(** [measure_time_float f]: returns a pair made of the result of [f()] and
   of the number of seconds taken by that call. *)
let measure_time_float (f : unit -> 'a) : 'a * float =
  let t0 = Unix.gettimeofday () in
  let res = f() in
  let t1 = Unix.gettimeofday () in
  res, (t1 -. t0)
...
(******************************************************************************)
(*                          Printing as OCaml code                            *)
(******************************************************************************)

let string_to_ocaml_code = Printf.sprintf "%S"

let option_to_ocaml_code sub_printer x =
  match x with
  | None -> "None"
  | Some x -> Printf.sprintf "(Some %s)" (sub_printer x)

let list_to_ocaml_code elt_printer l =
