...
let spec : (string * Arg.spec * string) list = [
     ...
     ("-f", Arg.Set force, " ensures that the input file is parsed even if it is already cached");
     ("--root", Arg.Set_string optitrust_root, " path to optitrust root")
   ]
...
let verbose_info msg =
  (if !verbose then Printf.ksprintf !Tools.info_fun else Printf.ifprintf ()) msg
...
let parse (filename: string) : unit =
  ...
  let is_up_to_date =
    if !force then begin
      ...
      false
    end else if not (Xfile.is_newer_than ser_filename filename) || not (Xfile.is_newer_than ser_filename program_path) then begin
      ...
      try
        ...
        if List.for_all (Xfile.is_newer_than ser_filename) deps then begin
          if !verbose
