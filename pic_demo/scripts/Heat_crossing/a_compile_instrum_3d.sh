...
###################################
#In our tests, 200 million particles are enough to give accurate percentages (we do not care about performance for this test). You can put more if you have more memory, to get more accurate percentages. Please check the maximum memory on one NUMA node of your architecture, then open Pic-Vert/memory/memory-3d.ods and check the resulting maximum number of particles possible, then change the following line accordingly if you want.
#As example, 2 billions is a maximum for 96 GB of memory.
nb_particles=200000000
...
###################################
compile_gcc() {
  cd $PICVERT_HOME/3d_heat_instrum_runs
  mkdir -p proportion$1-drift$2-vth$3
  export OMPI_CC=gcc
  mpicc -I$PICVERT_HOME/include $PICVERT_HOME/src/matrix_functions.c $PICVERT_HOME/src/meshes.c $PICVERT_HOME/src/output.c $PICVERT_HOME/src/parameter_reader.c $PICVERT_HOME/src/random.c $PICVERT_HOME/src/space_filling_curves.c $PICVERT_HOME/src/diagnostics.c $PICVERT_HOME/src/fields.c $PICVERT_HOME/src/initial_distributions.c $PICVERT_HOME/src/particle_type_concurrent_chunkbags_of_soa_3d.c $PICVERT_HOME/src/poisson_solvers.c $PICVERT_HOME/src/rho.c $PICVERT_HOME/simulations/instrum_sim3d_aocosoa.c -DSPARE_LOC_OPTIMIZED -DOMP_TILE_SIZE=2 -DNCX=64 -DNCY=64 -DNCZ=64 -DDELTA_T=0.02 -DINITIAL_DISTRIBUTION=DRIFT_VELOCITIES_3D -DPROPORTION_FAST_PARTICLES=$1 -DDRIFT_VELOCITY=$2 -DTHERMAL_SPEED=$3 -DNB_PARTICLE=$nb_particles -DNB_ITER=$nb_iterations -DCHUNK_SIZE=256 -DPIC_VERT_SHARED_BAGS -DPIC_VERT_CROSSING -lfftw3 -lm -O3 -fopenmp -march=native -std=gnu11 -o proportion$1-drift$2-vth$3/aocosoa_3d.out
}
...
compile_icc() {
  ...
  export OMPI_CC=icc
  mpicc -I$PICVERT_HOME/include $PICVERT_HOME/src/matrix_functions.c $PICVERT_HOME/src/meshes.c $PICVERT_HOME/src/output.c $PICVERT_HOME/src/parameter_reader.c $PICVERT_HOME/src/random.c $PICVERT_HOME/src/space_filling_curves.c $PICVERT_HOME/src/diagnostics.c $PICVERT_HOME/src/fields.c $PICVERT_HOME/src/initial_distributions.c $PICVERT_HOME/src/particle_type_concurrent_chunkbags_of_soa_3d.c $PICVERT_HOME/src/poisson_solvers.c $PICVERT_HOME/src/rho.c $PICVERT_HOME/simulations/instrum_sim3d_aocosoa.c -DSPARE_LOC_OPTIMIZED -DOMP_TILE_SIZE=2 -DNCX=64 -DNCY=64 -DNCZ=64 -DDELTA_T=0.02 -DINITIAL_DISTRIBUTION=DRIFT_VELOCITIES_3D -DPROPORTION_FAST_PARTICLES=$1 -DDRIFT_VELOCITY=$2 -DTHERMAL_SPEED=$3 -DNB_PARTICLE=$nb_particles -DNB_ITER=$nb_iterations -DCHUNK_SIZE=256 -DPIC_VERT_SHARED_BAGS -DPIC_VERT_CROSSING -lfftw3 -lm -O3 -qopenmp -march=native -std=gnu11 -o proportion$1-drift$2-vth$3/aocosoa_3d.out
}
