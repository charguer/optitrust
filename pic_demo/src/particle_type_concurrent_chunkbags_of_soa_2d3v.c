...
void bag_push_initial(bag* b, float dx, float dy, double vx, double vy, double vz) {
  chunk* c = b->front;
  int index = c->size++;
  c->dx[index] = dx;
  c->dy[index] = dy;
...
void init_all_chunks(int nb_bags_per_cell, long int num_particle, cartesian_mesh_2d mesh,
  ...
  int nb_chunks_per_cell[num_cells_2d];
  int cumulative_nb_chunks_per_cell[num_cells_2d];
  #pragma omp parallel
  ...
  int nb_full_tiles_x, nb_full_tiles_y;
  int remaining_cells_x, remaining_cells_y;
  ...
  spare_chunks_ids = allocate_int_matrix(nb_bags_per_cell, num_cells_2d);
  ...
  if (nb_bags_per_cell == 2) {
    ...
    int lower_x_border, lower_y_border;
    int upper_x_border, upper_y_border;
  ...
  number_of_spare_chunks_per_parity = cumulative_nb_chunks_per_cell[num_cells_2d - 1] + nb_chunks_per_cell[num_cells_2d - 1];
  
  // Computation of bijection between [0 ; nb_bags_per_cell[ x [0 ; num_cells_2d[ and [0 ; number_of_spare_chunks_per_parity[
...
void read_particle_array_2d3v(int mpi_rank, int mpi_world_size, long int num_particle, cartesian_mesh_2d mesh,
    ...
    *weight = (float)(mesh.x_max - mesh.x_min) * (float)(mesh.y_max - mesh.y_min) / ((float)mpi_world_size * (float)num_particle);
    ...
    for (i = 0; i < num_particle; i++) {
        ...
        bag_push_initial(&((*particles)[i_cell]), dx, dy, vx, vy, vz);
    }
    ...
    *weight = (float)x_range * (float)y_range / ((float)mpi_world_size * (float)num_particle);
    ...
    if (sim_distrib < INITIALIZE_VX_WITH_X_AND_Y) {
        ...
        for (j = 0; j < num_particle; j++) {
            ...
            bag_push_initial(&((*particles)[i_cell]), (float)(x - (int)x), (float)(y - (int)y), vx, vy, vz);
        }
    } else {
