...
(* [def vec_align tg]: expects the target [tg] to point at an array declaration, then 
    it will add the alignas attribute to its type with [vec_align] alignment size *)
...
(* [assume alignment var tg]: expects the target [tg] to be a relative target, then it
    will insert an instruction of the form var = __builtin_assume_aligned(var, alignment) 
    at that location.*)
