...
let parse_array spec argv first last =
  ...
  let rec parse i =
    if i <= last then begin
      ...
      match optact with
      | None ->
        let msg = Printf.sprintf "unknown argument `%s'" s in
        raise (CmdError msg)
      | Some(Set r) ->
          r := true; parse (i+1)
      | Some(Unset r) ->
      ...
      | Some(String fn) ->
          if i + 1 <= last then begin
            ...
            let msg = Printf.sprintf "option `%s' expects an argument" s in
            raise (CmdError msg)
          end
      | Some(Integer fn) ->
          if i + 1 <= last then begin
            let n =
              ...
              with Failure _ ->
                let msg = Printf.sprintf "argument to option `%s' must be an integer" s in
                raise (CmdError msg)
            in
            fn n; parse (i+2)
          end else begin
...
let long_int_action key s =
  ...
  try
    int_of_string s
  with Failure _ ->
    let msg =  Printf.sprintf "argument to option `%s' must be an integer" key in
    raise (CmdError msg)
