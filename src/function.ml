...
*)
let inline_call1 ?(name_result = "") ?(label:var = "__TEMP_body") ?(vars : rename = AddSuffix "1") ?(args : var list = []) ?(_no_control_structures : bool = true) (tg : Target.target) : unit =
(* DONE: use Target.apply_on_targets
   DONE: work with the explicit path converted to a target
   DONE: generate a nobrace sequence for producing the output *)
...
(* [inline_call ~name_result ~label ~vars ~args ~no_control_structures tg]
      ...
      of function call inlining.
*)
let inline_call ?(name_result = "") ?(label:var = "__TEMP_body") ?(vars : rename = AddSuffix "1") ?(args : var list = []) ?(_no_control_structures : bool = true) (tg : Target.target) : unit =
  let t = Trace.get_ast() in
