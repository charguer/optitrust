...
(* [replace_return exit_label r t] if the founded return statement is not terminal than replace it with an
    ...
    returns:
      ...
      TODO: return also the number of gotos created
      TODO: return also a boolean indicating whether there exists a return in depth
         that is, not flat in the sequence
*)
(* TODO: rename to process_return_in_inlining *)
let replace_return (exit_label : label) (r : var) (t : trm) : trm =
  let rec aux (is_terminal : bool) (*TODO: argument 'is_in_depth' true for the first call, then false *)(t : trm) : trm =
    match t.desc with
    | Trm_abort ab ->
      begin match ab with
      | Ret t1 ->
...
let inline_call_aux (index : int) (label : string) (top_ast : trm) (p_local : path ) (t : trm) : trm =
  ...
  | Trm_seq tl ->
   ...
   nb_gotos := 0;
   let labelled_body = begin match name with (* TODO: if then else *)
                       | "" ->
                          trm_labelled label fun_decl_body
                       | _ -> trm_labelled label (replace_return "__exit_body" name fun_decl_body)
                       end in
   let exit_label = begin match !nb_gotos with (* TODO: if then else *)
                    | 0  -> trm_lit (Lit_unit)
                    | _ -> trm_labelled "__exit_body" (trm_lit (Lit_unit))
                    end in
   let inlined_body = (* TODO:  if has_type_unit fun_decl_type then *)
