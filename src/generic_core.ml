  ...
  let rec aux (global_trm : trm) (t : trm) : trm = 
    ...
    | Trm_let (vk, (y, ty), init) ->
      if contains_variable x init 
        then trm_seq ~annot:(Some (No_braces (Nobrace.current()))) (List.mapi (fun i name ->
         trm_let vk (y ^ "_" ^(string_of_int i), ty) (Internal.change_trm (trm_var x) (trm_var name) init)) names)
        else t
    | Trm_apps (_, _) -> 
      if contains_variable x t then
        trm_seq ~annot:(Some (No_braces (Nobrace.current()))) (List.map (fun name -> Internal.change_trm (trm_var x) (trm_var name) t) names)
        else t 
...
let delocalize_aux (array_size : string) (neutral_element : int) (fold_operation : string) (t : trm) : trm =
  ...
  | Trm_seq tl ->
    ...
    let new_decl = trm_seq ~annot:(Some (No_braces (Nobrace.current())))[
    ...
    in
    let accum = trm_seq ~annot:(Some (No_braces (Nobrace.current()))) [
