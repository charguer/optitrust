...
(* [cparser_of_string s]: returns the parser that matches string [s]. *)
...
(* [string_of_cparser p]: prints parser [p]. *)
...
(* [get_selected ~parser ()]: gets the selected parser. *)
...
(* [select p]: selects [p] as parser to be used. *)
let select (p : cparser) : unit =
  selected_cparser := p

(* [select_by_string s]: selects the parser with name [s] as parser to be used. *)
let select_by_string (s : string) : unit =
  select (cparser_of_string s)

(* [select_if_not_default p]: selects [p] as parser to be used, unless [p] is [Default], in this case do nothing. *)
