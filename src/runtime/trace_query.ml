...
exception Trace_not_found
exception Trace_invalid
exception Trace_out_of_date

let read_trace_tree ?(timestamp: string option) (path: string) : step_tree =
  if not (Sys.file_exists path) then raise Trace_not_found;
  begin match timestamp with
    | Some timestamp ->
      let file_timestamp = string_of_float ((Unix.stat path).st_mtime) in
      if file_timestamp <> timestamp then raise Trace_out_of_date
    | None -> ()
  end;
  try
