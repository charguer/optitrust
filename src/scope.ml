...
(** Like [find_interference], but either traces a justification, or fails. *)
let assert_no_interference ~(after_what : string) ~(on_interference : string) tl_new_scope tl_after : unit =
  ...
  | [] -> Trace.justif (sprintf "variables are not used after %s" after_what)
  | [x] -> failwith (sprintf "variable '%s' is used after %s, but will now be %s" x after_what on_interference)
  | xs -> failwith (sprintf "variables %s are used after %s, but will now be %s" (Tools.list_to_string ~sep:"', '" ~bounds:["'";"'"] xs) after_what on_interference)

(** If [x] is used in [instrs], traces a justification, otherwise fails. *)
let assert_unused_in (x : var) (instrs : trm mlist) : unit =
...
(** Given a path to a variable definition, assert that it is unused using [assert_unused_in]. *)
let assert_unused (p : Path.path) : unit =
  ...
  assert_unused_in x instrs_after