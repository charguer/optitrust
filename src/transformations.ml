        ...
        | Trm_seq[f_loop;del_inst_f] ->
          ...
          | Trm_for (init,cond,step,body) -> trm_seq ~annot:(Some Delete_instructions) 
            [ trm_for init cond step 
              (change_trm (trm_var new_var) (trm_apps (trm_binop Binop_array_access) [trm_var new_var;trm_apps ~annot:(Some Heap_allocated) (trm_unop Unop_get) [trm_var "my_core_id"]]) body);
              del_inst_f
            ]
