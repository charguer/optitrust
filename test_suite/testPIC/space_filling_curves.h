...
int COMPUTE_I_CELL_2D(int ncy, int i_cell_x, int i_cell_y) {
  return ((i_cell_x) * ncy + (i_cell_y));
}
static inline int I_CELL_PARAM_2D(int ncx, int ncy) { return ncy; }
#elif (I_CELL_2D_TYPE == TILE) // Tile (no precalc)
int COMPUTE_I_CELL_2D(int num_cells_per_column, int i_cell_x, int i_cell_y) {
  return (TILE_SIZE * (i_cell_x) + ((i_cell_y) & TILE_SIZE_MINUS_ONE) + num_cells_per_column * ((i_cell_y) / TILE_SIZE));
}
...
int COMPUTE_I_CELL_2D(int *precalc_table, int i_cell_x, int i_cell_y) {
  return (TILE_SIZE * (i_cell_x) + ((i_cell_y) & TILE_SIZE_MINUS_ONE) + num_cells_per_column * ((i_cell_y) / TILE_SIZE))
}
static inline int I_CELL_PARAM_2D(int ncx, int ncy) { return ncx * TILE_SIZE; }
/*
...
int COMPUTE_I_CELL_2D(int throw_away, int i_cell_x, int i_cell_y) {
  return (dilate_2_notable(i_cell_x) + (dilate_2_notable(i_cell_y) << 1));
}
static inline int I_CELL_PARAM_2D(int ncx, int ncy) { return 0; }
#elif (I_CELL_2D_TYPE == MORTON_PRECALC) // Morton (with lookup table)
int COMPUTE_I_CELL_2D(int throw_away, int i_cell_x, int i_cell_y) {
  return (dilate_2(i_cell_x) + (dilate_2(i_cell_y) << 1));
}
static inline int I_CELL_PARAM_2D(int ncx, int ncy) { return 0; }
#elif (I_CELL_2D_TYPE == HILBERT) // Hilbert (auxiliary using Morton with lookup)
int COMPUTE_I_CELL_2D(int curve_order, int i_cell_x, int i_cell_y) {
  return encodeHilbert2d(curve_order, i_cell_x, i_cell_y);
}
