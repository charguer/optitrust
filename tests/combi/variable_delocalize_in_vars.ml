...
let _ =  Run.script_cpp ( fun () ->
  !! Variable.delocalize_in_vars ~index:"k" ~mark:"section_of_interest" ~old_var:"a" ~new_var:"x" ~var_type:(typ_str "T") ~array_size:"N" ~ops:(Delocalize_arith (Lit_int 0, Binop_add) ) ~local_vars:["xa";"xb"] [cFor "i"];
  !! Trace.alternative (fun () ->
    !! Variable.local_name  ~var_type:(typ_str "T") ~old_var:"a" ~new_var:"x" [cFor "i"];
