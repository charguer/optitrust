...
let _ = Run.script_cpp (fun () ->
  ...
  !! Function.inline ~name_result:"r" ~bind_args:false ["v1";"v2"] [cFun "vect_add" ~args_pred:(Target.target_list_one_st (cVar "v1"))];
  
  !! Struct.set_explicit [sInstrRegexp ~substr:true "speed2 ="];
  
  !! Function.inline ~name_result:"r" ~bind_args:true ["v1";"v2"] [cFun "vect_mul"];
  
  !! Function.inline ~name_result:"r" ~bind_args:true ["v1";"v2"] [cFun "vect_add"];
  
  (* TODO: Fix the issue with initialization list inside initialization list *)
  !! Struct.set_explicit [sInstr "pos2 ="];
  
  !! Function.inline ~name_result:"" ~bind_args:true ["b2";""] [cTopFun "main"; cFun "bag_push"]; 
