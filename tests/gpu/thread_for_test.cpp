...
void basic(int *a, int N, int M) {
  ...
  __DEF(r1, "MINDEX1(MSIZE2(N,M),0) ..+ MSIZE2(N,M)");
  __DEF(r2, "fun (i: int) -> (MINDEX2(N,MSIZE1(M),i,0) ..+ MSIZE1(M))");
...
void retile_desyncgroups(int *a, int N, int M) {
  ...
  __DEF(r1, "fun (sz: int) -> (MINDEX1(sz,0) ..+ sz)");
  __DEF(r2, "fun (i: int) -> (MINDEX2(N,MSIZE1(M),i,0) ..+ MSIZE1(M))");
  __DEF(r3, "fun (i: int) -> (MINDEX2(N*M/32,MSIZE1(32),i,0) ..+ MSIZE1(32))");
  ...
  __threadfor; for (int i = 0; i < N*M/32; i++) {
    __xconsumes("desync_for(r3(i)) j in ..32 -> &a[MINDEX2(N*M/32,32,i,j)] ~~>[GMem] 1");
    __xproduces("desync_for(r3(i)) j in ..32 -> &a[MINDEX2(N*M/32,32,i,j)] ~~>[GMem] 1+1");
    __threadfor; for (int j = 0; j < 32; j++) {
      __xconsumes("&a[MINDEX2(N*M/32,32,i,j)] ~~>[GMem] 1");
      __xproduces("&a[MINDEX2(N*M/32,32,i,j)] ~~>[GMem] 1+1");
      const int va = __gmem_get(&a[MINDEX2(N*M/32,32,i,j)]);
      __gmem_set(&a[MINDEX2(N*M/32,32,i,j)], va + 1);
    }
  }
...
void sync_required(int *a, int N, int M) {
  ...
  __DEF(r1, "fun (sz: int) -> (MINDEX1(sz,0) ..+ sz)");
  __DEF(r2, "fun (i: int) -> (MINDEX2(N,MSIZE1(M),i,0) ..+ MSIZE1(M))");
  __DEF(r3, "fun (i: int) -> (MINDEX2(M,MSIZE1(N),i,0) ..+ MSIZE1(N))");
  ...
  blocksync(); __with("H := desync_for(r1(MSIZE2(N,M))) i in ..N -> desync_for(r2(i)) j in ..M -> &a[MINDEX2(N,M,i,j)] ~~>[GMem] 1");
  __ghost(swap_groups, "items := fun i j -> &a[MINDEX2(N,M,i,j)] ~~>[GMem] 1");

  __ghost(rewrite_linear, "inside := fun sz -> ThreadsCtx(r1(sz)), by := msize_commute");
  __ghost(group_to_desyncgroup2, "D1 := N, D2 := M, items := fun i j -> &a[MINDEX2(N,M,j,i)] ~~>[GMem] 1");

  __threadfor; for (int i = 0; i < M; i++) {
    __xconsumes("desync_for(r3(i)) j in ..N -> &a[MINDEX2(N,M,j,i)] ~~>[GMem] 1");
    __xproduces("desync_for(r3(i)) j in ..N -> &a[MINDEX2(N,M,j,i)] ~~>[GMem] 1+1");
  ...
  __ghost(rewrite_linear, "inside := fun sz -> ThreadsCtx(r1(sz)), by := eq_sym(MSIZE2(N,M), MSIZE2(M,N), msize_commute)");
  blocksync(); __with("H := desync_for(r1(MSIZE2(M,N))) i in ..M -> desync_for(r3(i)) j in ..N -> &a[MINDEX2(N,M,j,i)] ~~>[GMem] 1+1");
  __ghost(swap_groups, "items := fun i j -> &a[MINDEX2(N,M,j,i)] ~~>[GMem] 1+1");
}

__DEF(rr1, "fun (sz: int) -> (MINDEX1(MSIZE1(sz),0) ..+ MSIZE1(sz))");
...
void read_thread_inner(int *a, int *b, int N) {
  ...
  __reads("KernelParams(MSIZE1(N), bpg, smem_sz)");
  __preserves("ThreadsCtx(rr1(N))");
  __writes("desync_for(rr1(N)) i in ..N -> &a[MINDEX1(N,i)] ~~>[GMem] reduce_sum(N,B)");
  __reads("for i in 0..N -> &b[MINDEX1(N,i)] ~~>[GMem] B(i)");
  ...
  for (int i = 0; i < N; i++) {
    __spreserves("desync_for(rr1(N)) t in ..N -> &a[MINDEX1(N,t)] ~~>[GMem] reduce_sum(i, B)");
    __threadfor; for (int t = 0; t < N; t++) {
