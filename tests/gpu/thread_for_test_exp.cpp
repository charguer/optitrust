...
// NOTE: using pretty matrix notation
 ...
 __device__ void basic (int* a, int N, int M)  {
  ...
  blocksync();
  __with("H := desync_for i in ..N -> desync_for j in ..M -> &a[i][j] ~~>[GMem] 1");
}
 ...
 __device__ void retile_desyncgroups (int* a, int N, int M)  {
  ...
  blocksync();
  __with("H := desync_for i in ..N -> desync_for j in ..M -> &a[i * M + j] ~~>[GMem] 1 + 1");
}
 ...
 __device__ void sync_required (int* a, int N, int M)  {
  ...
  blocksync();
  __with("H := desync_for i in ..M -> desync_for j in ..N -> &a[j][i] ~~>[GMem] 1 + 1");
...
__ghost(assert_inhabited, "x := arbitrary(int * (int -> int) -> int)", "reduce_sum <- x");

__ghost(assert_prop, "proof := admit(forall (f: int -> int) -> (0 = reduce_sum(0, f)))", "reduce_sum_empty <- proof");

__ghost(assert_prop, "proof := admit(forall (n: int) (f: int -> int) (_: (n >= 0)) -> (reduce_sum(n, f) + f(n) = reduce_sum(n + 1, f)))", "reduce_sum_add_right <- proof");
  ...
  __threadfor;  for (int t = 0; t < N; t++) {
    ...
    __ghost(rewrite_linear, "inside := fun v -> &a[t] ~~>[GMem] v, by := reduce_sum_empty(B)");
     for (int i = 0; i < N; i++) {
      ...
      const __ghost_fn focus = __ghost_begin(ro_matrix1_focus, "matrix := b, i := i");
