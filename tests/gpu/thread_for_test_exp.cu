...
__device__ void retile_desyncgroups(int __ctx_sz, int __tid, int* a, int N,
                                    int M) {
  const int __ctx_sz_0 = __ctx_sz / N;
  const int __i0 = __tid % __ctx_sz / __ctx_sz_0;
  const int __ctx_sz_1 = __ctx_sz_0 / M;
...
__device__ void sync_required_write_after_read(int __ctx_sz, int __tid, int* a,
                                               int N, int M) {
  const int __ctx_sz_0 = __ctx_sz / N;
  const int __i0 = __tid % __ctx_sz / __ctx_sz_0;
  const int __ctx_sz_1 = __ctx_sz_0 / M;
  const int __j1 = __tid % __ctx_sz_0 / __ctx_sz_1;
  const int __ctx_sz_2 = __ctx_sz / N;
  const int __i2 = __tid % __ctx_sz / __ctx_sz_2;
  const int __ctx_sz_3 = __ctx_sz_2 / M;
  const int __j3 = __tid % __ctx_sz_2 / __ctx_sz_3;
  const int __ctx_sz_4 = __ctx_sz / M;
  const int __i4 = __tid % __ctx_sz / __ctx_sz_4;
  const int __ctx_sz_5 = __ctx_sz_4 / N;
  const int __j5 = __tid % __ctx_sz_4 / __ctx_sz_5;
  a[MINDEX2(N, M, __i2, __j3)];
  __syncthreads();
  a[MINDEX2(N, M, __j5, __i4)] = 1 + 1;
  __syncthreads();
}
...
__device__ void read_thread_outer(int __ctx_sz, int __tid, int* a, int* b,
                                  int N) {
  const int __ctx_sz_0 = __ctx_sz / N;
...
__device__ void read_thread_inner(int __ctx_sz, int __tid, int* a, int* b,
                                  int N) {
  const int __ctx_sz_0 = __ctx_sz / N;
  const int __t0 = __tid % __ctx_sz / __ctx_sz_0;
