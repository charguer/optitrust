...
void requires_pure(int n) { __requires("(n = n)"); }
...
void ghost_pure(int m, int n) {
  ...
  for (int i = 0; i < m; i++) {
    ...
    ensures_not_ghost(6);
    ensures_not_ghost(7);
    __ghost(ensures_pure, "n := 1", "#_1 <- eq_refl");
    requires_pure(1);
    __ghost(ensures_pure, "n := 3", "#_2 <- eq_refl");
    requires_pure(3);
  }
  for (int i = 0; i < m; i++) {
    __strict();
  ...
  split:
    ...
    requires_pure(2);
    __ghost(ensures_pure, "n := 3", "#_3 <- eq_refl");
    requires_pure(3);
    __ghost(ensures_pure, "n := 4", "#_4 <- eq_refl");
    requires_pure(4);
    requires_pure(5);
    requires_pure(6);
  }
