...
void seq_ro_par_rw(int m, int n, int o, int* t) {
  ...
  for (int i = 0; i < m; i++) {
    ...
    __consumes(
        "_Uninit(Group(range(0, n, 1), fun j -> Group(range(0, o, 1), fun k -> "
        "&t[MINDEX3(m, n, o, i, j, k)] ~> Cell)))");
    __produces(
        "Group(range(0, n, 1), fun j -> Group(range(0, o, 1), fun k -> "
        "&t[MINDEX3(m, n, o, i, j, k)] ~> Cell))");
    for (int j = 0; j < n; j++) {
      __parallel_reads("&x ~> Cell");
      __consumes(
          "_Uninit(Group(range(0, o, 1), fun k -> &t[MINDEX3(m, n, o, i, j, "
          "k)] ~> Cell))");
      __produces(
          "Group(range(0, o, 1), fun k -> &t[MINDEX3(m, n, o, i, j, k)] ~> "
          "Cell)");
      for (int k = 0; k < o; k++) {
        __parallel_reads("&x ~> Cell");
...
void ghost_scope(int m, int n) {
  ...
  for (int i = 0; i < m; i++) {
    ...
    const __ghost_fn xfg =
        __ghost_begin(ro_fork_group, "H := &x ~> Cell, r := range(0, n, 1)");
    for (int j = 0; j < n; j++) {
      __reads("&x ~> Cell");
      int y = x;
    }
