...
void f(int N) {
  ...
  for (int i_s = 0; i_s < 2 * 10; i_s += 2) {
    __strict();
    __smodifies("&x ~> Cell");
    __ghost(assume, "F := in_range(exact_div(i_s, 2), 0..10)");
    x += i_s;
  }
  const int ratio = 5;
  ...
  for (int j = 0; j < ratio * N; j += ratio) {
    __strict();
    __smodifies("&x ~> Cell");
    __ghost(assume, "F := in_range(exact_div(j, ratio), 0..N)");
    x += j;
  }
}
...
void ghost_in_range(int N) {
  ...
  for (int i = 0; i < 4 * (N - 2); i += 4) {
    __strict();
    __smodifies("&x ~> Cell");
    __ghost(assume, "F := in_range(exact_div(i, 4), 0..N - 2)");
...
void arrays(int N, int* w, int* r, int* f) {
  ...
  for (int i = 0; i < 2 * N; i += 2) {
    ...
    __ghost(assume, "F := in_range(exact_div(i, 2), 0..N)");
