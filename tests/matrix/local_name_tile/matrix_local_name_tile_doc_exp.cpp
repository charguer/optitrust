...
void f() {
  ...
  __ghost([&]() {
    __consumes("b ~> UninitMatrix1(4)");
    __produces("for i1 in 3..7 -> &b[MINDEX1(4, i1 - 3)] ~> UninitCell");
    __admitted();
  ...
  __ghost([&]() {
    __consumes("for i1 in 3..7 -> &b[MINDEX1(4, i1 - 3)] ~> UninitCell");
    __produces("b ~> UninitMatrix1(4)");
    __admitted();
    __with("justif := shift_groups");
  });
  free(b);
