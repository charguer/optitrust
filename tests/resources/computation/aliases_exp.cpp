...
void array_copy_ref_alias(float* A, float* B, int n) {
  __preserves("B ~> Matrix1(n)");
  __reads("A ~> Matrix1(n)");
  for (int i = 0; i < n; ++i) {
    __strict();
    __xpreserves("&B[MINDEX1(n, i)] ~> Cell");
...
void dummy_modifies(int* p) { __preserves("p ~> Cell"); }
...
void eq_to_alias(float* A, int n) {
  __requires("(n = 1024)");
  __preserves("A ~> Matrix1(n)");
  __ghost(assert_alias, "x := n");
  for (int i = 0; i < 1024; ++i) {
    __strict();
    __xpreserves("&A[MINDEX1(1024, i)] ~> Cell");
