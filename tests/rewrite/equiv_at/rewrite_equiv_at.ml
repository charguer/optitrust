...
let _ = Run.script_cpp (fun _ ->
  ...
  !! Rewrite_basic.equiv_at "int a, k; ==> a + k * a == (k + 1) * a" [cWriteVar "res1"; dRHS];
  !! Rewrite_basic.equiv_at ~indepth:true "int a, k; ==> a + k * a == (k + 1) * a" [cVarDef "res2"];
  !! Rewrite_basic.equiv_at ~indepth:true "int a, k; ==> a + k * a == (k + 1) * a" [cVarDef "res4"];
