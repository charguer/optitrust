...
// OMP binding to get the current thread id
...
// extern worker_id_t thread_id_undef;

worker_id_t get_undef_thread_id();

worker_id_t get_thread_id_or_undef();
...
private:
  ...
  void check_index(index_type id) const {
    ...
    // TODO FOR OPTITRUST: revive this check:
...
public:
  ...
  value_type& mine(worker_id_t my_id) {
    assert(my_id == get_thread_id());
    return operator[](my_id);
  }

    /* TODO OPTITRUST REVIVE
...
public:
  ...
  value_type& mine() {
    return operator[](int(get_thread_id()));
  }
