...
(* NOT YET IMPLEMENTED *)
let flag_compare_expected_serialized : bool ref = ref false
...
let run_test ~(script_name:string) (test: unit -> (module TEST)) =
  ...
  with e ->
    ...
    if !Flags.tester_stop_on_error then raise Flags.Batching_interrupted_on_error
  end;
...
let batch_prelude ~(compare_expected_serialized : bool)
  ~(stop_on_error : bool) : unit = (* ARGUMENTS NOT USED *)
  ()
