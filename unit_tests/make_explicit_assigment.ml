...
let _ = 
    ...
    ( fun _ -> 
        ...
        *)
        ...
        show_path ~debug_ast:true [cStr "b = p"];
        ...
        (* make_explicit_record_assignment =  detach if needed + make_explicit_record_assignment_core *)
        (* make_explicit_record_assignment:  if path points to a vardef then call detach ~label:"detached" and then call make_explicit_record_assignment_core with path cLabel "detached" 
          else just call make_explicit_record_assignment_core on the given path *)
          (* vect b;   detached: b = v;                   instead of vect b =v *)
        dump()
    )
