...
Parameter var_eq_spec : forall (x y:var),
...
Parameter var_gen : varset -> var.
Parameter var_gen_spec : forall E, (var_gen E) \notin E.
Parameter var_fresh : forall (L : varset), { x : var | x \notin L }.
...
Proof using. apply (Inhab_of_val String.EmptyString). Qed.
...
Axiom var_gen : varset -> var.
Axiom var_gen_spec : forall E, (var_gen E) \notin E.
Axiom var_fresh : forall (L : varset), { x : var | x \notin L }.

(* DEPRECATED: adapt to strings
Definition varset := fset var.
...
Definition var_gen (E : varset) : var :=
...
Lemma var_fresh : forall (L : varset), { x : var | x \notin L }.
...
Fixpoint fresh (L : varset) (n : nat) (xs : list var) {struct xs} : Prop :=
  match xs, n with
  | nil, O => True
...
(** [gather_varset_for_type T F] return the union of all the finite sets
...
Ltac gather_varset_with F :=
  ...
  let L := gather (\{}:varset) in eval simpl in L.
